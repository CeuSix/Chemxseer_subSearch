...
/**
 * This is an implementation of the GXL converter:
 * Given a Graph data set in GXL format, we converted them into canonical DFS code format
 * Then this DFS code can be converted into Parmol Graph if necessary
 * @author dayuyuan
 *
 */
public class GXLConverter {
	...
	public Graph parse(File inputfile, String gName, GraphFactory factory) throws Exception{
		if(!inputfile.exists())
			...
			for(int i = 0; i< oneGraph.getGraphElementCount(); i++){
				...
				if(oneElement.getClass().getName().contains("GXLNode")){
					GXLAtomicValue xValue = (GXLAtomicValue)oneElement.getAttr("x").getValue();
					GXLAtomicValue yValue = (GXLAtomicValue)oneElement.getAttr("y").getValue();
					...
					GXLAtomicValue valenceValue = (GXLAtomicValue)theEdge.getAttr("valence").getValue();
					g.addEdge(nodexIndex2, nodexIndex1, Integer.parseInt(valenceValue.getValue()));	
				}
			}
			g.saveMemory();
			return g;
	...
	public int test(File inputfile, GraphFactory factory, int[] crossEdgeNum) throws Exception{
		if(!inputfile.exists())
			throw new IOException();
		else{
			GXLDocument gxlDoc = new GXLDocument(inputfile);
			...
			for(int i = 0; i< oneGraph.getGraphElementCount(); i++){
				...
				if(oneElement.getClass().getName().contains("GXLNode")){
					...
					GXLAtomicValue xValue1 = (GXLAtomicValue)node1.getAttr("x").getValue();
					GXLAtomicValue yValue1 = (GXLAtomicValue)node1.getAttr("y").getValue();
					GXLAtomicValue xValue2 = (GXLAtomicValue)node2.getAttr("x").getValue();
					GXLAtomicValue yValue2 = (GXLAtomicValue)node2.getAttr("y").getValue();
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i = 0; i< allfiles.length; i++){
			if(allfiles[i].isHidden())
				...
				crossRegionEdgeNum +=test.test(allfiles[i], MyFactory.getGraphFactory(), regionNodeNum);
			}
		}
		for(int i = 0; i< 16; i++){

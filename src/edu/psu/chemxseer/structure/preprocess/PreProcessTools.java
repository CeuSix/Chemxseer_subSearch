...
public class PreProcessTools{
	/**
	 * Given a database file downloaded from eMolecuels: <smiles id>
	 * Change the format of the file to: <id => smiles>
	 * @param eMoleculesFile
	 * @param SmilesFileName
	 * @throws IOException 
	 */
	public static void changeStyle(String eMoleculesFile, String SmilesFileName) throws IOException{
		System.out.println("IN ChangeSyle");
		String spliter = " => ";
		// Open InputFile
		BufferedReader fileBufReader = new BufferedReader(new FileReader(eMoleculesFile));	
		// Open OutputFile
		...
		while((lineString = fileBufReader.readLine())!=null){
			...
			if(lineToken[1].equals("11340785")
					...
					||lineToken[1].equals("712990") || lineToken[1].equals("713014") || lineToken[1].equals("716534"))
				continue;

			Graph g = null;
			try {
				g = MyFactory.getSmilesParser().parse(lineToken[0], MyFactory.getGraphFactory());
			} catch (Exception e) {
			...
			if(g == null)
				System.out.println("Things that santa do not want to see" + lineString);
			else if(!GraphConnectivityTester.isConnected(g))
				continue;
			outputWriter.write(graphCount++ + spliter + 
					lineToken[0] + "\n");
		}
		// Close input File
		try {
		...
		System.out.println("In processor: changeStyle, " + graphCount + " number of graphs" +
	...
	/**
	 * Given a file of graphs in SDF format
	 * Populate graphs in this file into A new graph file [with ID = > in Smiles format]
	 * Pay attention: all graphs have to be connected
	 * @param SDFFileName
	 * @param graphFactory
	 * @throws IOException 
	...
	public static void changeFormat(String SDFFileName, String SmilesFileName,
		...
		while((lineString = fileBufReader.readLine())!=null){
			...
			if(attStatus){
				...
				if(attStatus && lineString.equals("$$$$")){
					// Transform this graphString to a realGraph and save into database
					try {
		...
		System.out.println("In processor: changeFormat, " + index + " number of graphs" +
		"has been formated into Smiles Format");
		// Intrigue java garbage collector
		Runtime r = Runtime.getRuntime();
		r.gc();
	 ...
	 */
	public static void pruningUnconnected(String graphDBNameOld, String graphDBName) throws IOException, ParseException{
		...
		while ((line = bin.readLine()) != null) {
			...
			try {
				graphlists.add(sParser.parse(line.substring(pos + " => ".length()), line.substring(0, pos), 
	...
	public static void main(String[] args){
		String SDFFile = "/Users/dayuyuan/Downloads/NCI-Open_09-03.sdf";
		String SmilesFile = "/Users/dayuyuan/Downloads/NCI.smiles";
		try {
			PreProcessTools.changeFormat(SDFFile, SmilesFile, MyFactory.getGraphFactory());
		} catch (IOException e) {

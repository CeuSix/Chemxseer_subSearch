...
/**
 * the GSpanMiner_MultiClass is implemented via "recursion", 
 * which is hard to modify to support branch & bound
 * 
...
public class GSpanMiner_MultiClass_Iterative extends AbstractMiner{
	...
	//When a pattern get returned, denotes whether there are childIterator stored in the stack
	 ...
	 * @param inputGraphs
	 */
	public GSpanMiner_MultiClass_Iterative(Settings settings,List<ClassifiedGraph> inputGraphs) {
		super(settings);
		this.m_frequentSubgraphs = new FragmentSet(); // may not be used afterwards
    ...
    private void setUp2(Settings s, List<ClassifiedGraph> graphs) throws IOException{
    ...
    private void initializeOutput(){
		...
		for(int i = 0; i< deduction.length; i++){
			if(i == 0)
				deduction[i] = 0;
			else deduction[i] = deduction[i-1] + classGraphsCount[i-1];
		}
    }
	...
	/**
	 * Read One Edge, generate a DFS code for the edge
	 * Then call subgraph_mining 
	 * @param gs
	 */
	private FrequentFragment nextEdge(){
		...
		return subgraph_Mining(code); //recursive search // Dayu: modified by Dayu, by using the stack
	}


	/**
	 * Start of the Iterative mining: 
	 * put the "Iterator" in, return the current code.toFragment
	 * Return null if the code does not meet the constraints
	 * @param edgeCode the DFSCode with is found and checked for children
	 * @return the highest occurring frequency, of this branch  
	 */
	private FrequentFragment subgraph_Mining(DFSCode edgeCode) {
		if (!edgeCode.isMin()) {
			Debug.println(2, edgeCode.toString(m_settings.serializer)+" not min");
			m_settings.stats.duplicateFragments++;
			return null;
		}
		this.numberOfPatterns++;
		Iterator it = edgeCode.childIterator(m_settings.findTreesOnly, m_settings.findPathsOnly);
		if(this.iteratorStack.size()!=0) //at this time the iteratorStack must be empty
			System.out.println("Sometime is wrong");
		//if(edgeCode.getSubgraph().getEdgeCount() < m_settings.maximumFragmentSize)
		if(1 < m_settings.maximumFragmentSize)
			this.iteratorStack.push(it); // at the children iterator in stack
		if (m_settings.checkReportingConstraints(edgeCode.getSubgraph(), edgeCode.getFrequencies())){
	...
	/**
	 * (1) If the iteratorStack is empty, return null
	 * (2) Els if topIT, has next, construct a code and return, or return "null" if the code does not meet the constraints
	 * (3) topIT does not have next, pop the top iterator stack, and call subgraph_mining again
	 * @return
	 */
		...
		if(topIT.hasNext()){
			...
			if (code.isFrequent(m_settings.minimumClassFrequencies)){
				...
				if(code.getSubgraph().getEdgeCount() < m_settings.maximumFragmentSize){
					// Keep on searching "next"'s children only if only next is frequent & size < maxSize
					this.iteratorStack.push(
							code.childIterator(m_settings.findTreesOnly, m_settings.findPathsOnly));
					this.haveChildIterator = true;
				}
				else this.haveChildIterator = false;
				if (m_settings.checkReportingConstraints(code.getSubgraph(), code.getFrequencies())){
					return code.toFragment();
				}
				else return null;
			}
			else return null; // not frequent at all
		}
		else{
	...
	/**
	 * Return the next pattern in the search tree:
	 * Return null if no next pattern exists
	 * @return
	 */
	public FrequentFragment nextPattern(){
		...
		if(this.iteratorStack.isEmpty()){
			...
			else return null; // no edge left
		}
		else{
			result = this.subgraph_Mining();
		}
		// result = null may be caused by
		//1. The fragment returned does not meet criterion, thus return the next
		//2. The fragment can not be found, since this "edge" has no more children available, thus go to (1)
		if(result == null)
			return nextPattern();
		else return result;
	}
	 ...
	 * For the previous code get returned, there are two cases
	 * (1) The code's children will be enumerate next, since an new iterator was stored in the stack
	 * (2) Else, the code's children will not be enumerated. 
	 * in this case, the prune do nothing, we now we jump to another branch, then return false
	 */
    ...
    /**
     * Gien a FrequentFragment pattern enumerated from the enumerator
     * Construct and return a OneFeatureMultiClassWithPosting object
     * @param pattern
     * @return
     */
    public CoverSet_FeatureWrapper getFeature(FrequentFragment pattern){
    	//1. First Step: 
    	Graph[] supportingSet = pattern.getSupportedGraphs();
		int numofClasses =((ClassifiedGraph)supportingSet[0]).getClassFrequencies().length;
		...
		for(int i = 0; i< supportingSet.length;i++){
			float[] freq = ((ClassifiedGraph)supportingSet[i]).getClassFrequencies();
			for(int j = 0; j< freq.length; j++)
				if(freq[j]>0)
					// equal support
					if(pattern.getFragment().getEdgeCount() == supportingSet[i].getEdgeCount())
						supportingList[2*j+1].add(Integer.parseInt(supportingSet[i].getName())-deduction[j]);
					// containing support
					else 
						supportingList[2*j].add(Integer.parseInt(supportingSet[i].getName())-deduction[j]);
		}
		//2. Second Step:
		...
		OneFeatureMultiClass feature = new OneFeatureMultiClass(
				MyFactory.getDFSCoder().serialize(g), frequency, 
				shift, incrementalID++, false);
		
		return new CoverSet_FeatureWrapper(feature, thePostings);
    }
    /**************************BELOW IS THE STATIC FUNCTION USED TO CALL**********************************/
    /*****************************************************************************************************/
    
    /**
	 * Iterative Mining of the Search Space
	 * Sotre all the subgraphs meeting the constrain into m_frequentSubgraphs data member
	 */
     ...
     */
    private static PostingFeaturesMultiClass getFeatures(GSpanMiner_MultiClass_Iterative m, 
    		Settings s, String featureFileName, String[] postingFileNames) throws IOException{
		...
		if(postingFileNames != null){
			...
			for(int i = 0; i< postingChannels.length; i++){
				if(postingFileNames[i]!=null)
					postingChannels[i] = new FileOutputStream(postingFileNames[i]).getChannel();
			}
		}
		...
		for (Iterator it = m.m_frequentSubgraphs.iterator(); it.hasNext();) {
			...
			for(int i = 0; i< supportingSet.length;i++){
				...
				for(int j = 0; j< freq.length; j++)
					if(freq[j]>0)
						// equal support
						if(currentFragment.getFragment().getEdgeCount() == supportingSet[i].getEdgeCount())
							supportingList[2*j+1].add(Integer.parseInt(supportingSet[i].getName())-m.deduction[j]);
						// containing support
						else 
							supportingList[2*j].add(Integer.parseInt(supportingSet[i].getName())-m.deduction[j]);
			}
			...
			if(postingChannels!=null){
				for(int i = 0; i<  shift.length; i++){
					...
					shift[i] = 
						FeaturePosting.savePostings(postingChannels[i], thePosting, featureIndex);
				}
			}
			...
			OneFeatureMultiClass theFeature= 
				new OneFeatureMultiClass(s.serializer.serialize(currentFragment.getFragment()), 
		...
		NoPostingFeatures<OneFeatureMultiClass> features = 
			new NoPostingFeatures<OneFeatureMultiClass> (featureFileName, allFeatures, false);		
		
		return new PostingFeaturesMultiClass(postingFileNames, 
				features, m.classGraphsCount);
    }
    /**
     ...
     * @return
     */
    public static PostingFeaturesMultiClass gSpanMining(String[] args, String featureFileName, String[] postingFileNames)
    {
    	//1. Parse the input
    	Settings s =  parseInput(args);
    	//2. Mining
    	long startTime = System.currentTimeMillis();
    	GSpanMiner_MultiClass_Iterative m = new GSpanMiner_MultiClass_Iterative(s);
    	...
    	System.out.println("The Total Number of Patterns Enumerated: " + m.numberOfPatterns);
    	System.out.println("The Total Number of Patterns Found: " + m.m_frequentSubgraphs.size());
    	//3. return
    	try {
     ...
     * Why I need this? 
     * It's kind of wired, right? haha
     * @param args
     * @param postingFileName
     * @param maxNonSelectDepth
     * @return
     */
	public static PostingFeaturesMultiClass gSpanMining(String[] args, String[] postingFileNames) {
		return gSpanMining(args, null, postingFileNames);
	}
	
	/**
	 * the inputGraphs should be classified graphs
	 * the postingFileNames should be 2 times the inputGraphs classes
	 * A: posting for database graphs containing (not equal to) the feature
	 * B: posting for database graphs equal to the feature
	 * @param inputGraphs
	 * @param args
	 * @param featureFile
	 ...
	 * @return
	 */
	public static PostingFeaturesMultiClass gSpanMining(List<ClassifiedGraph> inputGraphs, String[] args, String featureFile, 
			String[] postingFileNames){
		//1. Parse the input
    	Settings s =  parseInput( args);
    	//2. Mining
    	long startTime = System.currentTimeMillis();
    	GSpanMiner_MultiClass_Iterative m = new GSpanMiner_MultiClass_Iterative(s, inputGraphs);
    	m.iterativeMining();
    	System.out.println("The Total Time Complexity: " + (System.currentTimeMillis()-startTime));
    	System.out.println("The Total Number of Patterns Enumerated: " + m.numberOfPatterns);
    	System.out.println("The Total Number of Patterns Found: " + m.m_frequentSubgraphs.size());
	 ...
	 * @return
	 */
	public static PostingFeaturesMultiClass gSpanMining(List<ClassifiedGraph> inputGraphs,String[] args, 
			String[] postingFileNames) {

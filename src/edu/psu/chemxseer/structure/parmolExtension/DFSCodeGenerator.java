...
public class DFSCodeGenerator {
	...
	public DFSCodeGenerator(Graph g, int maxEdgeNum){
		...
		DataBase gs = new DataBase(m_graphs, dayuFrequency, m_frequentSubgraphs, MyFactory.getGraphFactory());
			...
			if(!needtoGrow || !minCode || !canGrow){
				while(!this.allCodesit.isEmpty()){
					...
					if(sameLevelIt.hasNext())
					{
						...
						String testString = this.currentCode.toString(MyFactory.getDFSCoder());
				...
				if(itLevel == -1){
					...
					else this.currentCode = null;
				}
				
			}
			if(minCode)
				return label;
			else return nextCode();
		}		
	}
...
//		String gString = "C-C-S-C2-N(-C(-C1=N-C(-[F])(=N-C(-[F])(=N-1)))(=C3-C=2(-C=C-C=C-3)))(-C-C-C)";

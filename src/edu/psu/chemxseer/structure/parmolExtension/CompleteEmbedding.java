...
public class CompleteEmbedding implements GraphEmbedding, Graph, DirectedGraph, UndirectedGraph {
	...
	/**
	 * Create a new CompleteEmbedding given a supergraph and a subgraph
	 * and their corresponding mapping
	 * It is important to notice that this mapping do not ncessarily of being complete
	 * @param supergraph
	 * @param subgraph
	 * @param nodeMapping
	 */
	public CompleteEmbedding(Graph supergraph, Graph subgraph, int[] nodeMapping){
		...
		for(int i = 0; i< m_edgeCount; i++){
			int supergraphEdge = supergraph.getEdge(subgraph.getNodeA(i), subgraph.getNodeB(i));
			if(supergraphEdge!=-1)
	...
	public static int getCompleteEmbeddings2(Graph superGraph, Graph subGraph, Collection completeEmbeddings) {
		return getCompleteEmbeddings2(superGraph, subGraph, completeEmbeddings, Factory.instance);
	}
	...
	protected static int getCompleteEmbeddings2(Graph superGraph, Graph subGraph, Collection completeEmbeddings, Factory factory) {
		/*if (embedding instanceof NodeEmbedding) {
			completeEmbeddings.add(factory.getInstance(((NodeEmbedding) embedding).getNodeMapping(), embedding.getSubGraph(),
					embedding.getSuperGraph()));
			return 1;
			...
			if ((subGraph.getEdgeCount() > superGraph.getEdgeCount())
					&& (subGraph.getNodeCount() < superGraph.getNodeCount())) return 0;
			if ((subGraph.getEdgeCount() < superGraph.getEdgeCount())
					&& (subGraph.getNodeCount() > superGraph.getNodeCount())) return 0;
			...
			getSubgraphIsomorphism(new int[][] { subgraphNodes, supergraphNodes }, map, subGraph, 
					superGraph, new boolean[superGraph.getNodeCount()], 0, completeEmbeddings, factory);
			return completeEmbeddings.size() - size;
	}
	...
	/**
	 * Extended by Dayu Yuan:
	 * current embedding "this" is a embedding between subgraph and supergraph
	 * given another embedding, "to MidleEmbedding" is a embedding between subgraph and anothergraph
	 * return a embedding between another graph to supergraph if there is any
	 * @param toMidleEmbedding
	 * @return
	 */
	public List<CompleteEmbedding> getMidleEmbedding(CompleteEmbedding toMidleEmbedding){
		Graph subGraph = this.getSubGraph(), midleGraph = toMidleEmbedding.getSuperGraph(), 
				superGraph = this.getSuperGraph();
		...
		getSubgraphIsomorphism(new int[][] { midlegraphNodes, supergraphNodes }, map, midleGraph, 
				superGraph, used, count, embeddings, Factory.instance);

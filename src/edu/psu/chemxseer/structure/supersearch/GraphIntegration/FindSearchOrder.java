...
public class FindSearchOrder {
			...
			if(index == 0){
				...
				for(int i = 0; i< surroudingEdge.size(); i++){
					...
					if(freq == null)
						freq = 0;
					if(freq > maxFreq){
						// if both nodeA and nodeB of aEdge is visited, then continue;
						if(nodeVisited[aEdge.getNodeAID()] == true && nodeVisited[aEdge.getNodeBID()] == true){
							// remove the node
				...
				if(containued)
					...
					if(nodeVisited[maxEdge.getNodeBID()] == true)
						System.out.println("Excpetion in Integrate Graphs: getNodeSequence");
					//END OF TEST
					surroudingEdge.remove(maxID);
					results[index++] = maxEdge.getNodeBID();
					// update the status
					nodeVisited[maxEdge.getNodeBID()] = true;
					addSurroudings(g, maxEdge.getNodeBID(), surroudingEdge, nodeVisited);
	...
	private static void addSurroudings(Graph g, int node1, List<OneEdge> surroudingEdge, boolean[] nodeVisited){
		for(int w =0; w < g.getDegree(node1); w++){
			...
			if(nodeVisited[node2])
				...
				OneEdge aEdge = new OneEdge(node1, node2, edge, g.getNodeLabel(node1), g.getNodeLabel(node2), g.getEdgeLabel(edge));

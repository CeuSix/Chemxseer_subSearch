...
public class MergeGraph {
	/**
	 * Randomly Selected "count" number of graphs, and then merge them together to generated a synthesis graph and return
	 * @param gDB
	 * @param chooseN
	 * @throws IOException 
	 * @throws ParseException 
	 */
	public static Graph mergeGraphs(GraphDatabase gDB, int chooseN) throws ParseException, IOException{
		...
		int oriEdgeCount =0, oriNodeCount = 0;
		for(int i = 1; i< gs.length; i++){
			mg.insertGraph(result, gs[i]);
			oriEdgeCount += gs[i].getEdgeCount();
			oriNodeCount += gs[i].getNodeCount();
		}
		//System.out.println(" EdgeCount: " + oriEdgeCount + ", NodeCount: " + oriNodeCount);
		return result.getGraph();
	}
	
	public static void main(String args[]) throws ParseException, IOException{
		//String dbFileName = "/Users/dayuyuan/Documents/workspace/Experiment/feature";
		//GraphDatabase gDB = new GraphDatabase_InMem(new NoPostingFeatures(dbFileName, MyFactory.getFeatureFactory(FeatureFactoryType.MultiFeature)));
		String dbFileName = "/Users/dayuyuan/Documents/workspace/Experiment/DBFile_Raw";
		GraphDatabase gDB = new GraphDatabase_OnDisk(dbFileName, MyFactory.getSmilesParser());
		//TEST ONE:
		for(int i = 0; i < 100; i++){
			Graph mergedGraph = mergeGraphs(gDB, 5);
			//String gString = MyFactory.getSmilesParser().serialize(mergedGraph);
			//Graph newMergedGraph = MyFactory.getSmilesParser().parse(gString, MyFactory.getGraphFactory());
			String gString = MyFactory.getDFSCoder().serializeNonCanonical(mergedGraph);
			Graph newMergedGraph = MyFactory.getDFSCoder().parse(gString,MyFactory.getGraphFactory());
			//if(newMergedGraph.getEdgeCount()!=mergedGraph.getEdgeCount())
				//System.out.println("lala");
			System.out.println(" EdgeCount: " + mergedGraph.getEdgeCount() + ", NodeCount: " + mergedGraph.getNodeCount());
		}
		//TEST TWO:
	}
	/**
	...
	public void insertGraph(IntegrateGraph intG, Graph g){
		...
		boolean[] mappedIntegratedNode = new boolean[intG.getNodeCount() + g.getNodeCount()];
		...
		for(; nodeIndex< nodeSequence.length; nodeIndex++){
			if( nodeIndex== 0 ){
				OneEdge firstEdge = identifyFirstEdge(nodeSequence[0], nodeSequence[1], g, intG);
				if(firstEdge == null)
					return; // do nothing
				...
				int bigNodeB = identifyNode(nodeB, g, mapGraphNode, mappedIntegratedNode, intG);
				if(bigNodeB == -1)
					bigNodeB = intG.getNodeCount(); // no possible matching for the nodeB
				...
				for(int w = 0; w < g.getDegree(nodeB); w++){
					...
					if(bigNodeA == -1)
						...
						if(bigEdge == -1)
							bigEdge = intG.addEdge(bigNodeA, bigNodeB, g.getEdgeLabel(edge));
						OneEdge aEdge = new OneEdge(bigNodeA, bigNodeB, bigEdge, g.getNodeLabel(nodeA),
	 ...
	 * @return
	 */
	private int identifyNode(int newNode, Graph g, int[] smallToBig, boolean[] bigMapped, IntegrateGraph intG){
		//1. Find all the nodes that "nodeB" connected on graph g
		IntersectionSet set = null;
		...
		for(int w = 0; w < g.getDegree(newNode); w++){
			...
			if(smallToBig[adjNewNode] == -1)
				...
				List<Integer> candidateBig = new ArrayList<Integer>(); // based on one adjacent, find a list of candidates
				// find the adjacent node of nodeMap[nodeA] on the integrated node [not mapped only]
				...
				for(int bw = 0; bw < intG.getDegree(bigAdjNewNode); bw++){
					...
					if(bigMapped[bigNewNode]) //  this does not count
						continue;
					if(g.getNodeLabel(newNode) == intG.getNodeLabel(bigNewNode) && 
							g.getEdgeLabel(edge) == intG.getEdgeLabel(bigEdge)){
						candidateBig.add(bigNewNode);
						
						int freq = intG.getEdgeFreq(bigEdge);
						if(!nodeFreq.containsKey(bigNewNode) || nodeFreq.get(bigNewNode) < freq)
							nodeFreq.put(bigNewNode, freq);
					}
				}
				if(set == null){
					set = new IntersectionSet();
					set.addAll(candidateBig);
				}
				else
					set.retainAll(candidateBig);
				if(set.size() == 0)
					break;
			}
		}
	...
	/**
	 * Given the first & second node, return a edge [with the maximum frequency] on integrated graph
	 * @param nodeA
	 * @param nodeB
	 * @param g
	 * @return
	 */
	private OneEdge identifyFirstEdge(int nodeA, int nodeB, Graph g, IntegrateGraph intG){
		...
		for(int bigNode = 0; bigNode< intG.getNodeCount(); bigNode++){
			if( intG.getNodeLabel(bigNode)!= nodeALabel)
				...
				for(int w = 0; w < intG.getDegree(bigNode); w++){
					...
					if(intG.getNodeLabel(adjBigNode) == nodeBLabel && intG.getEdgeLabel(bigEdge) == edgeLabel){
						// found a matchable edge
						int freq = intG.getEdgeFreq(bigEdge);
						if(freq > maxFreq){
							maxFreq = freq;
							maxEdge = new OneEdge(bigNode, adjBigNode, bigEdge, nodeALabel, nodeBLabel, edgeLabel);
						}
					}
					else continue;
				}

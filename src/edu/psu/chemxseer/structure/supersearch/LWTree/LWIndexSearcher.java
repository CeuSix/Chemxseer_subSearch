...
/**
 * The index structure for the LWIndex: 
 * (1) The LWindex adopt the LindexSearch structure
 * (2) I can also return all the embeddings of between the maxSubgrpah features and the query
 * @author dayuyuan
 *
 */
public class LWIndexSearcher extends LindexSearcher implements IndexSearcherPrefix{
	//Given a query, the featureEmbeddings stores all the subgraphs of the query and their corresponding
	// fastSUEmbeddings
	...
	// For type 1 query, assigning the feature as a prefix of a graph will save ISO cost by prefix sharing
	// For type 2 query, assigning the feature as a prefix of a graph will filter out this "g"
	// Only Useful during the DTTreeSearcher Construction 
	// [posting construction, will be called by PostingBuilderLuceneVectorizerSingle]
	private int[] fSize;
	...
	/**
	 * For DJTreeSearcher Construction Only: 
	 * Instead of building the LindexSearcher, we also need to initialize type1 and type2 scored
	 * @param lindexSearcher
	 * @param features
	 * @param querySize
	 */
	...
	public FastSUCompleteEmbedding getEmbedding(int fID, Graph query) {
		...
		if(runISO){
			FastSUCompleteEmbedding emb = new FastSUCompleteEmbedding(this.getTermFullLabel(this.indexTerms[fID]), query);
			if(emb.issubIsomorphic() || emb.isIsomorphic()){
				this.featureEmbeddings.put(fID, emb);
				return emb;
			}
			else return null;
		}
		else return null; // may not happen
	}
	...
	public int getPrefixID(Graph g) {
		//Since the value is not initialized, there is no way of finding the prefix for this graph "g"
		...
		if(maxSubs.size()!=0 && maxSubs.get(0)==-1){
			...
			//2. Rank all those maximal subgraphs, and return the one with maximum gain function
			int result = -1;
			int maxGainValue = -1;
	...
	/**
	 * Given a featureID, find the gain function of 
	 * assigning a database graph g to this featureID's "benefit"
	 * @param featureID: prefix feature ID
	 * @param sizeG: size of the database graph [how much ISO test is saved while this database graph is not
	 * verified with ISO test]
	 * @return
	 */

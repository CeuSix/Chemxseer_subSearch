...
/**
 * Feature Feature Selector for the Supgraph Search: 
 * based on the maximum coverage model [the input set are weighted sets]
 * In this implementation, we use the simple greedy algorithm, in each iteration, the set with the 
 * maximum "gain" coverage is selected. 
 * Here I did not use the class that provided by the set-cover feature selection tool kits
 * Instead I am impliment new One
 * @author dayuyuan
 *
 */
public class MaxCover_FeatureSelector {
	 ...
	 * In each iteration, the gain is re-calculated, and the feature with the max-gain is selected
	 * @param frequentSubgraphs
	 * @param gDB
	 * @return
	 */
	public NoPostingFeatures<OneFeatureMultiClass> minePrefixFeatures(
			PostingFeaturesMultiClass frequentSubgraphs, GraphDatabase gDB){
		...
		NoPostingFeatures<OneFeatureMultiClass> features = frequentSubgraphs.getFeatures();
		...
		for(int i = 0; i< sets.length; i++){
			...
			ICoverSet_FeatureWrapper oneFeature = new CoverSet_FeatureWrapper2(frequentSubgraphs, i, -1);
		...
		// In each iteration, I select the feature with the maximum "gain", which is defined as the increase of the covered weight
		...
		System.out.println("(5) After Pattern Selection: " + selectedFeatureCount + " out of " + sets.length + " frequent patterns as selected for Indexing");
	...
	/**
	 * A more advanced implementation of the max-coverage feature selection
	 * An inverted index is first build
	 * The gain function of each features is "updated" instead of "re-caulcaulte". 
	 * In each iteration, the feature with the max-gain is selected, then the gain function of
	 * related feature is updated. The inverted index will help on finding the realted features
	 * whose score need to be updated. 
	 * @param frequentSubgraphs
	 * @param gDB
	 * @return
	 */
	public NoPostingFeatures minePrefixFeaturesWithInvertedIndex(PostingFeaturesMultiClass frequentSubgraphs, GraphDatabase gDB){
		System.out.println("Select Index-Patterns for LW-Index");
		System.out.println("(1) Features & DB Postings Are Stored in Memory");
		System.out.println("(2) Inverted-index are built to facilitate score upate");
		//1. Initialize the Status
		float[] status = new float[gDB.getTotalNum()];
		for(int i = 0; i< status.length; i++)
			status[i] = 0;
		float[] featureGain = new float[frequentSubgraphs.getFeatures().getfeatureNum()];
		...
		for(int i = 0; i< sets.length; i++){
			ICoverSet_FeatureWrapper oneFeature = new CoverSet_FeatureWrapper2(frequentSubgraphs, i, -1);
			CoverSet_Weighted oneSet = LWIndexSetConverter.featureToSet_Weight(oneFeature, gDB, querySize);
			sets[i] = oneSet;
		...
		System.out.println("(4) Time for Inverted-Index Building " + (System.currentTimeMillis()-buildInvertedIndexStart));
		MemoryConsumptionCal.runGC();
		memoryEnd = MemoryConsumptionCal.usedMemoryinMB();
		System.out.println("Total Memory After Inverted-Index Building: " + (memoryEnd-memoryStart));
			...
			for(int w = 0; w < selectedSet.size(); w++){
				...
				if(gScore <= status[gID])
					...
					for(int i = 0; i< scores[gID].length; i++){
						...
						else featureGain[gID] -= gScore-oldStatusScore;
					}
				}
			}
		}
		System.out.println("(5) Time for Pattern Selection: " + (System.currentTimeMillis()-start));
		System.out.println("(6) After Pattern Selection: " + selectedFeatureCount + " out of " + sets.length + " frequent patterns as selected for Indexing");
		//4. Return selected Features
		List<OneFeatureMultiClass> selectedFeatures = features.getSelectedFeatures();
		return new NoPostingFeatures<OneFeatureMultiClass>(selectedFeatures, true);
	}
	 ...
	 * @return
	 */
	private void buildInvertedIndex(int[][] index, float[][] scores, CoverSet_Weighted[] inputSets){
		//1. Initialization

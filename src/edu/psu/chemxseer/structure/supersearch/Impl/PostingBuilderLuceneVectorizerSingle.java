...
/**
 * Currently Only Support DFS parser:
 * More parsers will be added soon
 * @author dayuyuan
 *
 */
public class PostingBuilderLuceneVectorizerSingle implements PostingBuilderLuceneVectorizer{
	private IndexSearcherPrefix searcher;
	
	public PostingBuilderLuceneVectorizerSingle(IndexSearcherPrefix searcher){
		this.searcher = searcher;
	}
	
	/*
	 * Vectorize the Database Graph:
	 * (1) Prefix can be either -1 or an number >=0
	 * (2) The label of "g" is extension labeled if prefix >=0
	 * @see edu.psu.chemxseer.structure.subsearch.Interfaces.PostingBuilderLuceneVectorizer#vectorize(int, de.parmol.graph.Graph, java.util.Map)
	 */
	public Document vectorize(int gID, Graph g,  Map<Integer, String> gHash) throws ParseException {
		...
		if(gHash==null){
			...
			gDoc.add(new Field("subGraphs", byteString, Field.Store.YES, Field.Index.NOT_ANALYZED));
		}
		else if( prefixID!=-1){
			gDoc.add(new Field("subGraphs", gHash.get(prefixID), Field.Store.YES, Field.Index.NOT_ANALYZED));
		}
		else 
			gDoc.add(new Field("subGraphs", new Integer(-1).toString(), Field.Store.YES, Field.Index.NOT_ANALYZED));

...
public class CIndexTreeFeatureSelector {
	...
	public CIndexTreeFeatureSelector(PostingFeaturesMultiClass postingFeatures, int minQueryCount) 
		...
		for(int i = 0; i< featureCount; i++){
			this.FGMap[i] = postingFeatures.getFullPosting(i,0); // FGMap[i] = all DB Graphs containing f
		}
		...
		for(int i = 0; i < featureCount; i++){
			for(int j = 0; j< queryCount; j++)
				FQCover[i][j] = false;
			int[] temp = postingFeatures.getFullPosting(i, 1);
			for(int qID:temp)
				FQCover[i][qID] = true;
		}
		System.out.println("(1) Time for CIndexTreeFeatureSelector Construction:" + (System.currentTimeMillis()-startTime));
	}
	...
	public CIndexTreeFeatureNode constructFeatureTree(){
		...
		for(int i = 0; i< wholeQueries.length; i++)
			wholeQueries[i] = i;
		CIndexTreeFeatureNode root = CIndexTreeFeatureNode.ConstructBinaryFeatureTree(wholeQueries, this);
		
		//ConsumptionCal.runGC();
		double endMemory = MemoryConsumptionCal.usedMemoryinMB();
		System.out.println("(2) Space for CIndexFeature Mining: " + (endMemory-startMemory));
		System.out.println("(3) Time for CIndexTreeFeature Mining: " + (System.currentTimeMillis()-startTime));
		System.out.println("(4) After Pattern Selection: " + root.getNodeCount() + " out of " + features.getfeatureNum() + " frequent patterns as selected for Indexing");
		return root;
	}
	...
	/**
	 * Given the set of features, return the complement of Union of those features
	...
	/**
	 * get the feature score on query "qID".
	 * if feature contained in qID, return 0;
	 * else return the number of graphs get covered in the unFilteredGraphs
	 * @param aFeature
	 * @param queries
	...
	public int getFeatureScore(IOneFeature aFeature, int[] queries,
		...
		int fID = aFeature.getFeatureId();
		//1. Calculate how many queries does not contain aFeature, so that it can use aFeature for filtering
		int count = 0; 
		for(int i = 0; i< queries.length; i++){
			int qID = queries[i];
//			if(fID <0 || fID >= FQCover.length || qID <0 || qID >= FQCover[fID].length)
//				System.out.println("find it");
			if(this.FQCover[fID][qID] == false)
				count++;
		}
		//2. For those features use aFeature for filtering, it have the same score: 
		int eachScore = IntersectionSet.getInterSectionSize(unFilteredGraphs, this.FGMap[fID]);
		return count * eachScore;
	}
	
	/**
	 * Given the sorted queries, split the query into two arrays
	 * results[0] are the queries containing the feature
	 * results[1] are the queries not containing the feature
	 * @param queries
	 * @return
	 */
	public int[][] splitQueries(int[] queries, IOneFeature theFeature) {
		...
		for(int i = 0, containedIndex = 0, unContainedIndex = 0; i< queries.length; i++){
			...
			else results[1][unContainedIndex++] = qID;
		}
		return results;
	}

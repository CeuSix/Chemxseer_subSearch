...
/**
 * The flat index of the CIndex: super succinct 
 * All index and postings are stored in character format
 * 
 * @author dayuyuan
 *
 */
public class CIndexFlat implements IndexSearcherLucene {
	 ...
	 * Given the query graph q, find all the features that are subgraph isomorphic to the query
	 * @param q
	 * @return 
	 */
	public ArrayList<Integer> getNoSubgraphs(Graph q, long[] TimeComponent){
		...
		for(int i = 0; i< featureStatus.length; i++){
			Graph f = MyFactory.getDFSCoder().parse(indexingGraphs[i], MyFactory.getGraphFactory());
	 ...
	 * @return
	 */
	public ArrayList<Integer> getNoSubgraphs(int[] subgraphs, long[] TimeComponent){
		long start = System.currentTimeMillis();

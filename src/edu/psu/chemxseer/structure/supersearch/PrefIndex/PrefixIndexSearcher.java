 ...
 *
 */
public class PrefixIndexSearcher extends GraphDatabase
		implements IndexSearcherLucene , IndexSearcherPrefix{
	...
	public List<Integer> subgraphs(Graph query, long[] TimeComponent) {
		...
		for(int i = 0; i< this.features.length; i++){
			...
			if(f.getNodeCount() > query.getNodeCount() || f.getEdgeCount() > query.getEdgeCount())
				continue;
			
			FastSUCompleteEmbedding embedding = new FastSUCompleteEmbedding(f, query);
	...
	/**
	 * Not the Upper-Most Layer, subgraph search
	 * TimeComponent[2] is calculated as filtering cost
	 * @param query
	 * @param candidate
	 * @param embes
	 * @param TimeComponent
	 * @return
	 */
	public List<Integer> subgraphs(Graph query, List<Integer> candidate, Map<Integer, FastSUCompleteEmbedding> embes,
			long[] TimeComponent){
		this.internalQuery = query;
		this.allExamed = true;
		long start = System.currentTimeMillis();
		...
		for(Integer oneID : candidate){
			...
			if(prefFeature == null){
				...
				FastSUCompleteEmbedding prefEmb = embes.get(prefFeature.getFeatureID());
				FastSUCompleteEmbedding thisEmb = new FastSUCompleteEmbedding(prefEmb, oneFeature.getSuffix());
	...
	/**
	 * Given the query graph, return all the subgraphs features not subgraph isomorphic to the query
	 * The subEmbedding records all the subgraph features subgraph isomorphic to the query
	 * @param query
	 * @param TimeComponent
	 * @return
	 */
	...
	public ArrayList<Integer> nonSubgraphs(int[] subgraphs, long[] TimeComponent){
		long start = System.currentTimeMillis();
		int[] result = IntersectionSet.getCompleteSet(subgraphs, this.getFeatureCount());
	...
	public PrefixFeature getFeature(int prefixID) {
		...
		else return this.features[prefixID];
	}
	...
	public FastSUCompleteEmbedding getEmbedding(int fID, Graph query) {
		...
		if(runISO){
			...
			if(featureGraph.getEdgeCount() > query.getEdgeCount() || featureGraph.getNodeCount() > query.getNodeCount())
				return null;
			FastSUCompleteEmbedding emb = new FastSUCompleteEmbedding(featureGraph, query);

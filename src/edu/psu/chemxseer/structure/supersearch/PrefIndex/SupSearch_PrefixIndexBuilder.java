...
public class SupSearch_PrefixIndexBuilder {
	/**
	 * Construct a Single Layer of the SupSearch_PrefixIndex:  no lowerLevel, no upperLevel
	 * May or may not have upperLevelindex
	 * @param features
	 * @param gDB
	 * @param baseName
	 * @return
	 ...
	 */
	public SupSearch_PrefixIndex buildIndex(NoPostingFeatures features, GraphDatabase gDB, 
		...
		PrefixIndexSearcher index = PrefixIndexSearcherConstructor.construct(features);
		System.out.println("(1) Time for PrefixIndex Construction: " + (System.currentTimeMillis()-startTime));
		PrefixIndexSearcherConstructor.saveSearcher(index,baseName, SupSearch_PrefixIndex.getIndexName());
		//2. Build the Prefix-index postings
		startTime = System.currentTimeMillis();
		PostingBuilderLuceneVectorizerMultiField postBuilder = new PostingBuilderLuceneVectorizerMultiField(index, index);
		...
		System.out.println("(2) Time for PrefixIndex lucene Construction: " + (System.currentTimeMillis()-startTime));
		//3. Build the Verifier
		VerifierISOPrefix prefixISO = new VerifierISOPrefix(index, false);
		PostingFetcherLucene postFetcher = new PostingFetcherLucene(lucenePath, gDB.getTotalNum(), 
...
//		//END OF TEST
	 ...
	 * @throws IOException
	 */
	public SupSearch_PrefixIndex loadIndex(GraphDatabase gDB, String baseName, boolean lucene_in_mem)
		throws NumberFormatException, IOException{
		PrefixIndexSearcher searcher =PrefixIndexSearcherConstructor.loadSearcher(baseName, SupSearch_PrefixIndex.getIndexName(),null);
		PostingFetcher posting = new PostingFetcherLucene(baseName + SupSearch_CIndexFlat.getLuceneName(), 
	...
	protected SupSearch_PrefixIndex buildIndex(PrefixIndexSearcher index,
			GraphDatabase gDB, String baseName, boolean lucene_in_mem) throws CorruptIndexException, LockObtainFailedException, IOException {
		//2. Build the Prefix-index postings
		long startTime= System.currentTimeMillis();
		PostingBuilderLuceneVectorizerMultiField postBuilder = new PostingBuilderLuceneVectorizerMultiField(index, index);
		PostingBuilderLucene builder = new PostingBuilderLucene(postBuilder);
		String lucenePath = baseName + SupSearch_PrefixIndex.getLucene();
		builder.buildLuceneIndex(lucenePath,index.getFeatureCount(), gDB, null);
		System.out.println("Level 0: Time for Lucene Index Construction: " + (System.currentTimeMillis()-startTime));
		//3. Build the Verifier
		VerifierISOPrefix prefixISO = new VerifierISOPrefix(index, false);
		PostingFetcherLucene postFetcher = new PostingFetcherLucene(lucenePath, gDB.getTotalNum(), 
				MyFactory.getDFSCoder(), lucene_in_mem);
	...
	protected SupSearch_PrefixIndex loadIndex(PrefixIndexSearcher searcher,
			GraphDatabase gDB, String baseName, boolean lucene_in_mem) throws NumberFormatException, IOException {
		PostingFetcher posting = new PostingFetcherLucene(baseName + SupSearch_CIndexFlat.getLuceneName(), 
				gDB.getTotalNum(), MyFactory.getDFSCoder(), lucene_in_mem);
		VerifierISOPrefix prefixISO = new VerifierISOPrefix(searcher, false);
		return new SupSearch_PrefixIndex(searcher, posting, prefixISO);

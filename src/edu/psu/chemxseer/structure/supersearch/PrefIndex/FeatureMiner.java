...
public class FeatureMiner {
	/**
	 * Prefix Feature Selection Algorithm:
	 * In each iteration, we select the feature with the maximum "gain"
	 * The gain function is defined as: size(f) * (covered-1)/covered
	 ...
	 * @return
	 */
	public PostingFeatures minPrefixFeatures(PostingFeatures frequentSubgraphs, int dbCount){
		...
		System.out.println("(3.1) Base Sort Features: " + (System.currentTimeMillis()-startTime));
		...
		while(selectedCount < features.getfeatureNum()){
			float maxGain = -1;
			OneFeatureImpl nextF = null;
			for(int fID = features.getfeatureNum()-1; fID >=0; fID--){
				OneFeatureImpl oneF = features.getFeature(fID);
				if(oneF.isSelected())
					continue;
				else if(oneF.getFeatureGraph().getEdgeCount() < maxGain) // gain  < edgeCount < maxGain
					break;
		...
		frequentSubgraphs.discardInMemPosting(); // discard the in-memory postings to save space

...
/**
 * A Class Similar to the LindexTerm, but does not have children
 * Only have "prefixID"
 * @author dayuyuan
 *
 */
public class PrefixFeature {
	...
	/**
	 * Construct a Prefix Feature
	 * The input featureString is in DFS code format
	 * @param featureString
	 */
	public PrefixFeature(int featureID, String featureString, PrefixFeature prefixFeature){
		this.featureID = featureID;
		this.featureSuffix = MyFactory.getDFSCoder().parseTextToArray(featureString);
		this.prefixFeature = prefixFeature;
	}
	/**
	 * Given the string representation of the PrefixFeature, and the indexSearcher(can be null)
	 * Construct a PrefixFeature
	 * @param prefixFeatureString
	 * @param indexSearcher
	 */
	public PrefixFeature(String prefixFeatureString, PrefixIndexSearcher indexSearcher){
		String[] tokens = prefixFeatureString.split(",");
		this.featureID = Integer.parseInt(tokens[0]);
		this.featureSuffix= MyFactory.getDFSCoder().parseTextToArray(tokens[1]);
...
//				System.out.println("Exception: Cannot find the prefixFeature: Error in PrefixFeature Construction");
		...
		else this.prefixFeature = indexSearcher.getFeature(prefixID);
	}
	/**
	 ...
	 */
	public PrefixFeature(int featureID, Graph g, PrefixIndexSearcher upperLevelIndex) {
		this.featureID = featureID;
		int prefixID = upperLevelIndex.getPrefixID(g);
		if(prefixID!=-1){
			//1. the emb is the embedding from the prefix to the graph g
			FastSUCompleteEmbedding emb = upperLevelIndex.getEmbedding(prefixID, g);
			...
			this.featureSuffix = iso.graphExtensionLabeling(suState).getExtension();
		}
		else{
			this.prefixFeature = null;
...
//	 * @param prefix
//	 */
//	public FastSUStateLabelling assignPrefix(PrefixFeature prefix, FastSUStateLabelling prefixISO){
...
//				return null;
//			}
//			else labelling = iso.graphExtensionLabeling(prefix.featureSuffix, this.featureSuffix);
//		}
//		else labelling = iso.graphExtensionLabeling(prefixISO, this.featureSuffix);
...
//	}
	...
	public Graph getGraph() {
		if(this.prefixFeature !=null){
			...
			return MyFactory.getDFSCoder().parse(wholeLabel, MyFactory.getGraphFactory());
		}
		else return MyFactory.getDFSCoder().parse(this.featureSuffix, MyFactory.getGraphFactory());
	}
	...
	public int[][] getWholeLabel(){
		...
		else return this.featureSuffix;
	}
	...
	public String toString(){
		String featureString = MyFactory.getDFSCoder().writeArrayToText(this.featureSuffix);

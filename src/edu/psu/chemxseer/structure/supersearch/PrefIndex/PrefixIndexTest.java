...
public class PrefixIndexTest {
	...
	public void buildHiIndex() throws IOException{
		...
		selectedFeatures[0] = miner.minPrefixFeatures(freqFeatures, gDB.getTotalNum());
		//1.2 mine second level features
		freqFeatures = FeatureProcessorFG.frequentSubgraphMining(new GraphDatabase_InMem(selectedFeatures[0].getFeatures()), baseName + "feature_1", 
				baseName + "posting1", 0.01, 10, gDB.getParser());
		selectedFeatures[1] = miner.minPrefixFeatures(freqFeatures, selectedFeatures[0].getFeatures().getfeatureNum());
	...
	public void buildIndex() throws CorruptIndexException, LockObtainFailedException, IOException{
		...
		PostingFeatures freqFeatures = FeatureProcessorFG.frequentSubgraphMining(gDB, 
				baseName + "feature", baseName + "posting", 0.01, 10, gDB.getParser()) ;
		//2. Mine Prefix Features
		FeatureMiner miner = new FeatureMiner();
		NoPostingFeatures selectedFeatures = miner.minPrefixFeatures(freqFeatures, gDB.getTotalNum()).getFeatures();
		//3. Build the Index
...
//		}
	...
	public void runHiQueries() throws NumberFormatException, IOException, ParseException{
		String baseName = base + "PrefIndexHi/";
		//1. Load the 2-level hierachical index
		NoPostingFeatures dbFeatures = new NoPostingFeatures(dbFile, MyFactory.getFeatureFactory(FeatureFactoryType.OneFeature));
		GraphDatabase gDB = new GraphDatabase_InMem(dbFeatures);
		SupSearch_PrefixIndexHiBuilder builder = new SupSearch_PrefixIndexHiBuilder();
		SupSearch_PrefixIndexHi index = builder.loadIndex(baseName, 2, gDB, false);
		//2. Query processing
		GraphDatabase query = new GraphDatabase_OnDisk(queryFile, MyFactory.getSmilesParser());
		AIDSExp.runQueries(query, index);
//		FastSU iso = new FastSU();
...
//		}
	...
	public static void main(String[] args) throws CorruptIndexException, LockObtainFailedException, IOException, NumberFormatException, ParseException{

...
public class CRGraphSearcher implements IndexSearcherLucene{
	protected CRGraphNode [] allNodes;
	protected boolean[] visitedStatus;
	protected IndexSearcherPrefix FGPTree;
	
	protected CRGraphSearcher(CRGraphNode[] allNodes, IndexSearcherPrefix FGPTree){
	...
	public List<Integer> subgraphs(Graph query, long[] TimeComponent) {	
		...
		for(int i = 0; i< allNodes.length; i++){
			...
			if(prefix == -1){
				...
				FastSUCompleteEmbedding newEmbs = new FastSUCompleteEmbedding(aNode.getGraphLabel(), query);
				if(newEmbs.issubIsomorphic())
					results.add(i);
				else this.onLineSheding(aNode);
			}
			else{
				FastSUCompleteEmbedding embes = FGPTree.getEmbedding(prefix, query);
				if(embes == null)
					...
					else newEmbs = new FastSUCompleteEmbedding(embes, aNode.getGraphLabel());
					
					if(newEmbs.issubIsomorphic())
						results.add(i);
					else this.onLineSheding(aNode); // aNode is not subgraph isomorphic to the query, none of its children will
				}
			}
	...
	/**
	 * Return the Minimum Non Subgraph of the Query q
	 * Since the CRGraphs are ordered 
	 * @param query
	 * @param TimeComponent
	 * @return
	 */
	public List<Integer> getNonSubgraphs(Graph query, long[] TimeComponent){
		...
		//1.Search for subgraphs
		List<Integer> subgraphs = this.subgraphs(query, TimeComponent);
		long startTime = System.currentTimeMillis();
		Collections.sort(subgraphs);
		//2. Get the complimentary set
		for(int i = 0, j = 0; i< this.allNodes.length; i++){
			if(j >= subgraphs.size())
				result.add(i);
			else if( i == subgraphs.get(j)){
				j++;
				continue;
			}
			else if( i < subgraphs.get(j))
				result.add(i);
			else System.out.println("Error in getNoSubgraphs of CRGraphSearcher");
		}
		TimeComponent[2] += System.currentTimeMillis()-startTime;
		return result;
	}
	...
	/**
	 * Mark all the visitedStatus[] of seeds descendants (including seed) to be true
	 * @param visitedStatus
	 * @param seed
	 */

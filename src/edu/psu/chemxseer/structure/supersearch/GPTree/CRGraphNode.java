...
/**
 * Represent a node of the DAG of the CRGraph:
 * (1) Different from the LindexTerm: Labels has no extension relationships
 * (2) Different from the PrefixFeature: 
 * Labels has no extension relationships, but the nodes have child/parent realtionships
 * @author dayuyuan
 *
 */
public class CRGraphNode implements Comparable<CRGraphNode> {
	...
	/**
	 * Build a CRGraphNode with no children nor prefix.
	 * The graphLabel is the full graph label. Extension Labelling will be constructed soonner
	 * @param nodeID
	 * @param graphLabel
	 */
		...
		if(tokens.length == 3)
			...
			for(int i = 3; i< tokens.length; i++){
				...
				if(children[i-3] == null)
					System.out.println("Exception in the Construction of CRGraphNODE from the nodeString");
			}
		}
	}
	/**
	...
	public void reLabelGraph(int[][] prefix, int prefixID) {
		...
		FastSUStateLabelling labeling = iso.graphExtensionLabeling(prefix, graphLabel);
		this.graphLabel = labeling.getExtension(); 
		// the graphLabel can be null since the feature is the same as its prefix
		this.prefixID = prefixID;
	}

	/**
	 * The prefix is not the CRGraphNode parent, but the prefix feature in prefixTree
	 * @return the prefixID
	 */
	...
	public Graph getGraph(int[][] prefix) {
		...
		else labels = graphLabel;
		return MyFactory.getDFSCoder().parse(labels, MyFactory.getGraphFactory());
	}
	/**

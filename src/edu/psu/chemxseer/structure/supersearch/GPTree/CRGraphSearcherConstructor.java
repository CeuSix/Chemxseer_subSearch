...
public class CRGraphSearcherConstructor{
	...
	public static CRGraphSearcher construct(NoPostingFeatures_Ext significantFeatures, 
			IndexSearcherPrefix FGPTree){
		//1. Construct all the CRGraphNode
		CRGraphNode[] allNodes = new CRGraphNode[significantFeatures.getfeatureNum()];
		for(int i = 0; i< allNodes.length; i++){
			String dfsCode =significantFeatures.getFeature(i).getDFSCode();
			allNodes[i] = new CRGraphNode(i,MyFactory.getDFSCoder().parseTextToArray(dfsCode));
		}
		...
		// Just order all the nodes according to their underlying graph size is OK. 
		// From small to big. 
		Arrays.sort(allNodes);
		//1.4 Set the ID
		for(int i = 0; i< allNodes.length; i++)
			allNodes[i].setNodeID(i);// reset the IDs
		//2: Prefix Extension: Need to label each CRGraphnode as a suffix of a feature in the FGPTree
		for(int i = 0; i< allNodes.length; i++){
			int prefixID = FGPTree.getPrefixID(allNodes[i].getGraph(null));
			if(prefixID == -1) // no prefixID, not all graphs are covered? it may happen
				continue;
			else{
	 ...
	 * @throws IOException
	 */
	public static CRGraphSearcher loadSearcher(IndexSearcherPrefix FGPTree, String baseName, String indexName) throws NumberFormatException, IOException{
		BufferedReader reader = new BufferedReader(new FileReader(new File(baseName, indexName)));
	 ...
	 * @throws IOException
	 */
	public static void saveSearcher(CRGraphSearcher searcher, String baseName, String indexName) throws IOException{
		...
		for(int i = 0; i< searcher.allNodes.length; i++){
			if(status[i])
				continue;
			else saveIndex(writer, searcher.allNodes[i], status);
		}
		writer.close();
	}
	
	private static void saveIndex(BufferedWriter writer, CRGraphNode node, boolean[] status) throws IOException{
		CRGraphNode[] children = node.getChildren();
		if(children !=null && children.length > 0){
			for(CRGraphNode oneC : children) {
				if(status[oneC.getNodeID()])
					continue;
				else saveIndex(writer, oneC, status);
			}
		}

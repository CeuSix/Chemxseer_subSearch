 ...
 *
 */
public class FeatureMiner {
	/**
	 * Given the frequent subgraphs, mine the significant features
	 * @param frequentSubgraphs
	 * @param ratio: 1/0.8 for default value
	 * @return
	 * @throws ParseException 
	 */
	public PostingFeatures minSignificantFeatures(PostingFeatures frequentSubgraphs, 
			double ratio) throws ParseException{
		...
		System.out.println("(2) Space for Pattern Selection: " + (memoryEnd-memoryStart));
		//1. Mine the containment relationships between frequent subgraphs
		long startTime = System.currentTimeMillis();
		NoPostingFeatures_Ext<OneFeatureImpl> freqFeaturesExt = 
			new NoPostingFeatures_Ext<OneFeatureImpl>(frequentSubgraphs.getFeatures());
		freqFeaturesExt.mineSubSuperRelation();
		freqFeaturesExt.sortFeatures(new FeatureComparatorBase());
		System.out.println("(3.1) Mine Containment Relationships: " + (System.currentTimeMillis()-startTime));
			...
			if(children == null || children.size() == 0){
				...
				for(int w =0; w < children.size(); w++){
					if(children.get(w).isSelected())
						set.addAll(frequentSubgraphs.getPosting(children.get(w)));
				}
		...
		System.out.println("(3.2) Time for Pattern Selection: " + (System.currentTimeMillis()-startTime));
		System.out.println("(4) After Pattern Selection: " + selectedCount + " out of " + freqFeaturesExt.getfeatureNum()+ " frequent patterns as selected for Indexing");
		return frequentSubgraphs.getSelectedFeatures(null, null, false);
	}
	
	/**
	 * GPTree Feature Selection Algorithm:
	 * In each iteration, the largest features "covering at least two items" are selected
	 * @param frequentSubgraphs: Pay attention, in the original work, only "edge-reduced" subgraphs are mined
	 * @param dbCount: to total number of database graphs
	 * @return
	 */
	public NoPostingFeatures minPrefixFeatures(PostingFeatures frequentSubgraphs, int dbCount){
		...
		long startTime = System.currentTimeMillis();
		NoPostingFeatures<OneFeatureImpl> features = frequentSubgraphs.getFeatures();
		...
		for(int fID = features.getfeatureNum()-1; fID >=0; fID--){
			OneFeatureImpl oneF = features.getFeature(fID);
			...
			else oneF.setUnselected();
		}
		System.out.println("(3) Time for Pattern Selection: " + (System.currentTimeMillis()-startTime));
		System.out.println("(4) After Pattern Selection: " + selectedCount + " out of " + features.getfeatureNum()+ " frequent patterns as selected for Indexing");
		//3. Return the Features
		List<OneFeatureImpl> selectedFeatures = features.getSelectedFeatures();
		return new NoPostingFeatures<OneFeatureImpl>(selectedFeatures, false);
	}
}

...
//		String dirName = "/Users/dayuyuan/Documents/workspace/Experiment/";
		...
		for(int i = 1; i<=5; i++){
			...
			PostingFeatures freqFeatures = 
				FeatureProcessorFG.frequentSubgraphMining(baseName + oriQuery, featureFileName, 
						null, 0.005, 50, MyFactory.getDFSCoder());
			GraphDatabase rawDB = new GraphDatabase_InMem(freqFeatures.getFeatures());
			RandomChoseDBGraph.randomlyChooseDBGraph(rawDB, 10000, baseName + dbFileName);
			//2. Get statistics: 
			//System.out.println("Statistics of the overall query file");
			GraphDatabase queryGraphs = new GraphDatabase_OnDisk(baseName + oriQuery, MyFactory.getDFSCoder());
			...
			RandomChoseDBGraph.randomlySplitDBGraph(queryGraphs, 8000, baseName + trainQueryName, baseName + testQueryName);
			//4. Build the Index & Run the Index
//			GraphDatabase trainingDB = new GraphDatabase_OnDisk(baseName + dbFileName, MyFactory.getDFSCoder());
//			GraphDatabase trainQuery = new GraphDatabase_OnDisk(baseName + trainQueryName, MyFactory.getDFSCoder());
//			System.out.println("Build LWIndex");
//			LWIndexExp.buildIndex(trainingDB, trainQuery, trainingDB, MyFactory.getDFSCoder(), baseName);
...
//			CIndexExp.buildIndexTopDown(trainingDB, trainQuery, trainingDB, MyFactory.getDFSCoder(), baseName);
		}

...
/**
 * This class is in charge of preparing the experiment data
 * (1) Build the graph database: subgraphs of the queries
 * (2) Randomly sampling the queries, divide into "training" and "testing" queries
 * (3) Generate Synthetic queries & graph database
 * @author dayuyuan
 *
 */
public class DataPreparation {
	...
	public static void AIDSDataset() throws IOException{
		...
		GraphDatabase qDB = new GraphDatabase_OnDisk(queryName, MyFactory.getSmilesParser());
		String dbFileName = baseName + "DBFile";
		//1. Construct the graph database, mine frequent subgraphs with minimum support 0.5%, maximum support 10%
		// 0.001% failed. 
		//PostingFeatures dbFeature = FeatureProcessorFG.frequentSubgraphMining(queryName, dbFileName+"raw", null, 0.005, 50, MyFactory.getSmilesParser());
		NoPostingFeatures dbFeatures = new NoPostingFeatures( dbFileName+"raw", MyFactory.getFeatureFactory(FeatureFactoryType.OneFeature));
		...
		for(int i = 10; i >0; i--){
			...
			RandomChoseDBGraph.randomlyChooseDBGraph(gDB, chooseNum, dbFileName+ i);
			gDB = new GraphDatabase_OnDisk(dbFileName +i, MyFactory.getDFSCoder());
		}
	}
	
	public static void prePareQueries() throws ParseException, IOException, MathException{
		...
		GraphDatabase gDBRw = new GraphDatabase_OnDisk(queryName, MyFactory.getSmilesParser());
		RandomChoseDBGraph.randomlyChooseDBGraph(gDBRw, 10000, sampleQueryName);
		GraphDatabase gDB = new GraphDatabase_OnDisk(sampleQueryName, MyFactory.getSmilesParser());
		RandomChoseDBGraph.randomlySplitDBGraph(gDB, 8000, trainQuery, testQuery);
	...
	/**
	 * Filtering the Raw Subgraphs: 
	 * (1) minFreq =< freq =< maxFreq
	 * (2) minEdge =< edge =< maxEdgeCount
	 * @param rawDBFileName
	 * @param newDBFileName
	 * @param minFreq
	 * @param maxFreq
	 ...
	 * @throws IOException
	 */
	private static void filterFrequentSubgraphs(String rawDBFileName, 
			String newDBFileName, int minFreq, int maxFreq, int minSize, int maxSize) throws IOException{
		//1. Load the Features
		NoPostingFeatures rawFeatures = new NoPostingFeatures(rawDBFileName, MyFactory.getFeatureFactory(FeatureFactoryType.OneFeature));
		//2. Filter the too frequent subgraphs
		for(int i = 0; i< rawFeatures.getfeatureNum(); i++){
			IOneFeature aFeature = rawFeatures.getFeature(i);
			int edgeCount = aFeature.getFeatureGraph().getEdgeCount();
			if(aFeature.getFrequency() > maxFreq || aFeature.getFrequency() < minFreq)
				aFeature.setUnselected();
			else if(edgeCount > maxSize || edgeCount < minSize)
					aFeature.setUnselected();
			else aFeature.setSelected();
		}
		NoPostingFeatures newFeatures = new NoPostingFeatures(rawFeatures.getSelectedFeatures(), false);
		newFeatures.saveFeatures(newDBFileName);
	}
	
	/**
	 * Results: The edge follows normal distribution (min edge = 10, max = 18 for minsupport 0.01)
	 * The frequency follows exponential distribution (with the decrease of the min support, the total number of 
	 * frequent subgraph grows exponentially, or according to power law. )
	 * 
	 * @param featureName
	 */
	public static void getStatistics(String featureName){
		NoPostingFeatures rawFeatures = new NoPostingFeatures(featureName, MyFactory.getFeatureFactory(FeatureFactoryType.OneFeature));
		...
		for(int i = minEdge; i <= maxEdge; i++){
			...
			else System.out.println(edgeCount.get(i));
		}
		System.out.println("FreqCount: " + minFreq + "," + maxFreq);
		for(int i = minFreq; i <= maxFreq; i++){
			if(freqCount.get(i) == null)
				System.out.println(0);
			else System.out.println(freqCount.get(i));
		}
	}
	
	/**
	 * Based on the experiment results:
	 * The peak is on minEdge + 25, the distribution is basically normal distribution with mean = minEdge +25, 
	 * [min, min+25, min+50].
	 * min = 1 
	 * So I can generate 3 data set: normal distribution with mean 15, 25, 35
	 * @param gDB
	 */
	public static void getStatistics(GraphDatabase gDB){
		HashMap<Integer, Integer> edgeCount = new HashMap<Integer, Integer>();
		int minEdge = Integer.MAX_VALUE, maxEdge = 0;

...
/**
 * Implementation of indexing features: 
 * Each index feature is associated with a list of maximal queries
 * The starting point score can be calculated and returned for each indexing features
 * @author dayuyuan
 *
 */
public class IndexFeatureInfo implements Comparable<IndexFeatureInfo>{
	 ...
	 * @param iternalID: the ID of the underling feature
	 */
	...
	/**
	 * 
	 * @param internalID: the ID of the underlying feature
	 * @param maxQueries
	 * @param queries
	 */
	public IndexFeatureInfo(int internalID, List<QueryInfo> maxQueries, List<QueryInfo> queries){
	...
	/**
	 * This must be called before any getUpperBound
	 * Calculate the upper bound according to the formula
	 * The upper bound is calculated over all queries 
	 */
	...
	/**
	 * This must be called before any getAvg or getMedian or getQuatile or isGoodRoot
	 * Given the featureInfo, recalculate the average and median
	 * of the "gain upper bound" function for each maxQueryInfo 
	 * For now the "gain upper bound" = gaps.size();
	 */
	...
	/**
	 * Return the # of queries has this index feature, multiply with "frequencies"
	 * @return
	 */

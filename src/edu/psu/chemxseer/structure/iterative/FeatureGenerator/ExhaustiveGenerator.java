...
/**
 * The branch and bound algorithm: exhaustive BB
 * No Query Grouping at all
 * @author duy113
 *
 */
public class ExhaustiveGenerator  extends AbstractMiner {
	...
	private void subgraph_Mining(DFSCode code, float[] freq) {
		...
		for (; it.hasNext();) {
			...
			if (next.isFrequent(m_settings.minimumClassFrequencies)
				...
				if(cInfo!=null && cInfo.getBranchUpperBound() < this.optimumGain)
	...
	/**
	 * Exhaustive Search (Branch & Bound) with minimum support
	 * Finding a CandidateFeature
	 * @param minFreq
	 ...
	 */
	public static CandidateInfo genCandidate(double minFreq, int maxNonSelectDepth,
			FeatureFinder selector, QueryInfo[] queries) throws FileNotFoundException,
	 ...
	 */
	protected void readGraphs(QueryInfo[] queries) throws IOException, ParseException {
		long t = 0;

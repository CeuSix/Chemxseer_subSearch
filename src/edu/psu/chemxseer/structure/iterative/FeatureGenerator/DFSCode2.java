 ...
 * Change A Little Bit: 
 * 
 * string from the first edge: edge.next is the next edge added into the code
 * for each rootGraph string: edge = edge.next.prev
 * for newly added rootGraph string: edge.prev = previous edge that was attached to brunch
 * And it equals to null, if it is the first edge attached to this brunch
 */
public class DFSCode2 implements Comparable{
	...
	protected GSpanEdgeFeatureSelection[] parents; // Denote the parent of each node
	// If the node[i] is a rootNode, then parents[i] is just the edge connecting this node
	// If the node[i] is a newly grown node, then parents[i] is the edge growing this node, 
	// Stop searching, 
	...
	/**
	 * Create a new DFSCode representing the one-edge graph containing the given Edge
	 * @param rootGraph: Important, the rootGrap is only a linked list representation, there is no guarantee
	 * that this rootGraph is in its canonical form, only requirement in one edge, if nodeA < nodeB, that means a forward edge
	 * 
	 * @param edge
	...
	public DFSCode2(int[][] rootGraph,  DataBaseModified dataBase, GraphSet myset){
		...
		for(int i = 0; i< rootGraph.length; i++){
			...
			root[i] = new GSpanEdgeFeatureSelection(nodeA, nodeB, labelA, edgeLabel, labelB, -1);
		...
		for(int i = 0; i< this.lasts.length; i++) lasts[i] = null;
		this.lastNodes = new int[this.rootNodeCount];
		for(int i = 0; i< lastNodes.length; i++) lastNodes[i] = i;
		this.dataBase= dataBase;
		parents=new GSpanEdgeFeatureSelection[dataBase.maxNodeCount];
		for(int i =0; i< parents.length; i++) parents[i] = null;
		//root[0] = null;
		...
		if (dataBase!=null){
			...
			for(int i = 0; i< rootGraph.length; i++){
				...
				else this.graph.addEdge(nodeA, nodeB, edgeLabel); // backward
			}
		}
	...
	private void searchChildren(GSpanGraph dataBaseGraph,
		...
		for(int w = 0; w < this.rootNodeCount; w++){
			...
			else lastNodeLabel = this.parents[lastNode].labelB;
			//extensions at the last node: true last Nodes, or starting nodes of each brunch
			...
			for (int i=0;i<ggraph.getDegree(nodeG);i++){
				...
				if (usedEdges[edge]==Graph.NO_EDGE){
					...
					if(usedNodes[oNode] == Graph.NO_NODE)
						gEdge = new GSpanEdgeFeatureSelection(lastNode,totalNodeCount,lastNodeLabel,
							ggraph.getEdgeLabel(edge),ggraph.getNodeLabel(oNode), w);
					
					else if(usedNodes[oNode] < lastNode)
						gEdge = new GSpanEdgeFeatureSelection(lastNode,usedNodes[oNode],lastNodeLabel,
							ggraph.getEdgeLabel(edge),ggraph.getNodeLabel(oNode), w);
					else continue;
					if ( (parents[lastNode] ==null ||parents[lastNode].nodeB < this.rootNodeCount  
							|| (lasts[w].compareTo(gEdge)<0)) 
			...
			//extensions at the right most path: until meet brunch starting point
			for (GSpanEdgeFeatureSelection ack=parents[lastNode];ack!=null && ack.nodeB >= rootNodeCount;ack=parents[ack.nodeA]){
				nodeG=ackNodes[ack.nodeA];
				for (int i=0;i<ggraph.getDegree(nodeG);i++){
					int edge=ggraph.getNodeEdge(nodeG,i);
					int oNode=ggraph.getOtherNode(edge,nodeG);
					if (usedEdges[edge]==Graph.NO_EDGE 
							&& usedNodes[oNode] == Graph.NO_NODE // only forward edges are allowed
							&& (!findPathsOnly || graph.getDegree(ack.nodeA)==1))		// paths have max degree 2
					{
						GSpanEdgeFeatureSelection gEdge=new GSpanEdgeFeatureSelection(ack.nodeA,totalNodeCount,
								ack.labelA,ggraph.getEdgeLabel(edge),ggraph.getNodeLabel(oNode), w);
						GraphSet s=(GraphSet)childEdges.get(gEdge);
						if (s==null){
							s=new GraphSet();
							childEdges.put(gEdge,s);
			...
			else nodeLabelW = this.parents[lastNode].labelB;
			for (int i=0;i<ggraph.getDegree(nodewG);i++){
				int edge=ggraph.getNodeEdge(nodewG,i);
				int oNode=ggraph.getOtherNode(edge,nodewG);
				if (usedEdges[edge]==Graph.NO_EDGE && usedNodes[oNode] < w && usedNodes[oNode] !=Graph.NO_NODE){
					GSpanEdgeFeatureSelection gEdge=new GSpanEdgeFeatureSelection(w,usedNodes[oNode],
							nodeLabelW ,ggraph.getEdgeLabel(edge),ggraph.getNodeLabel(oNode), -1);
					GraphSet s=(GraphSet)childEdges.get(gEdge);
					if (s==null){
						s=new GraphSet();
						childEdges.put(gEdge,s);
	 ...
	 * @param findPathsOnly 
	 */
	private void searchEmbedding(GSpanEdgeFeatureSelection currentEdge, GSpanGraph dataBaseGraph, 
			int[] ackNodes, int[] usedNodes, int[] usedEdges, 
			boolean findTreesOnly, boolean findPathsOnly){
		MutableGraph ggraph=dataBaseGraph.me;
		if (currentEdge==null){ // no more edges to add, so embedding is found
			...
			for (int i=0;i<ggraph.getDegree(node);i++){ // try all edges of the corresponding node
				...
				if (usedEdges[edge]==-1 && ggraph.getEdgeLabel(edge)==currentEdge.edgeLabel){
					...
					if (ggraph.getNodeLabel(oNode)==currentEdge.labelB && usedNodes[oNode]==Graph.NO_NODE){
		...
		public Iterator childIterator(boolean findTreesOnly, boolean findPathsOnly){ //=enumerate
			if (childEdges==null){
				...
				for(int i =0; i< ackNodes.length; i++) ackNodes[i] = Graph.NO_NODE;
				for (Iterator ggit=myset.iterator();ggit.hasNext();){ //search all database graphs for children
					...
					for (int i=0;i<ackGraph.getNodeCount();i++) usedNodes[i]=Graph.NO_NODE;
			...
			return new Iterator() { //a tricky iterator, creates no new DFSCodes, but change and restore this DFSCode 
		...
		private GraphSet add(GSpanEdgeFeatureSelection edge,GraphSet dbSubSet){
			...
			// The first time that one edge is attached to the brunch: add this edge without hesitation
		...
		private GraphSet remove(GSpanEdgeFeatureSelection edge, GraphSet set){
			...
			if (bLastEdge.compareTo(edge)!=0) // should not happend, if correctly use of childIterator() 
			...
			if (dataBase!=null){
				...
				if (edge.nodeA<edge.nodeB) {
					...
					else this.lastNodes[attachedNode] = bpEdge.nodeB;

...
/**
 * Search for the optimum features (starting from the rootGraph) with optimumGain, 
 * If an branch has upperBound < optimumGain, then prune the branch, stop searching. 
 * 
 * Attention: this is the Query Grouping approache, in which only a partial set of queries are used
 * @author dayuyuan
 *
 */
public class GroupQueryGenerator extends AbstractMiner{
	...
	protected void startRealMining() {
		...
		DataBaseModified gs = new DataBaseModified(m_graphs, m_settings.minimumClassFrequencies,
	...
	private void graphSet_Projection(DataBaseModified gs) {
		...
		for(Iterator it = gs.frequentEdges(); it.hasNext();){
			edu.psu.chemxseer.structure.parmol.gSpanInduced.GSpanEdge edge = 
				(edu.psu.chemxseer.structure.parmol.gSpanInduced.GSpanEdge) it.next();
			GraphSet temp = gs.getContainingGraphs(edge);
			allGraphs.addAll(temp);
		}
		DFSCode2 code = new DFSCode2(this.rootGraph, gs, allGraphs); // create DFSCode for the
		// current edge
		...
		for (; it.hasNext();) {
			...
			if (next.isFrequent(m_settings.minimumClassFrequencies)){
				if(next.toFragment().getFragment().getEdgeCount() <= this.maxMustNonSelectSize) {
					...
					CandidateInfo cInfo = selector.makeCandidateInfo(codeG, next.toFragment().getSupportedGraphs());
					// Use fake upper bound for filtering
					if(cInfo!=null && cInfo.getUpperBound() < this.optimumGain)
	 ...
	 */
	public static CandidateInfo genCandidate(double minFreq, int maxNonSelectDepth, int[][] rootGraph, 
			FeatureFinder selector, List<QueryInfo> queries, int optimumGain) throws FileNotFoundException,
			IOException, ParseException, InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		
		String gDBFileName = null;
		String[] args = {"-minimumFrequencies="+(-minFreq), "-maximumFragmentSize="+maxNonSelectDepth,
				"-graphFile="+gDBFileName,
				"-closedFragmentsOnly=flase", "-outputFile=temp", "-parserClass=de.parmol.parsers.SmilesParser", 
	 ...
	 * @throws ParseException
	 */
	protected void readGraphs(List<QueryInfo> queries) throws IOException, ParseException {
		long t = 0;

...
/**
 * This class is in charge of generating queries
 * and select queries with very big gap or very big candidate set
 * @author duy113
 *
 */
...
//		gen.generateInFrequentQueriesUniform2(4, 10, 1000, gDB, edgeIndex);
	 ...
	 * @param index
	 * @param queryFile
	 * @param byGapSize: true then prune by gap size, false then prune by candidate size
	 * @throws Exception
	 */
	public void gapsFiltering(HashMap<Graph, Integer> qMaps, 
			SubgraphSearch_LindexUpdate index, String queryFile, boolean byGapSize) throws Exception{
		...
		for(Entry<Graph, Integer> oneEntry : qMaps.entrySet()){
			...
			List<Integer> maxSubs = index.indexSearcher.maxSubgraphs(g, TimeComponent);
			if(maxSubs.get(0)==-1)
				continue;
			else{
				GraphFetcher candidateFetcher = index.getCandidate(maxSubs);
				if(byGapSize){
					int[] gaps = index.getGap(g, candidateFetcher);
					queries.add(new TestQuery(g, gaps.length, oneEntry.getValue()));
				}
				else queries.add(new TestQuery(g, candidateFetcher.size(), oneEntry.getValue()));
			}
		}
	...
	/**
	 * Gaps filtering for the Fgindex, since all the subgraphs > upperBound can be answered directly by FGindex
	 * So here, we discard those kind of index
	 * @param qMaps
	 * @param index
	 * @param queryFile
	 * @param uppBound
	 ...
	 * @throws Exception
	 */
	public void gapsFiltering(HashMap<Graph, Integer> qMaps, SubgraphSearch_LindexUpdate index, 
			String queryFile, int uppBound, boolean byGraphSize) throws Exception{
		List<TestQuery> queries = new ArrayList<TestQuery>();	
		long[] TimeComponent = new long[4];
		for(Entry<Graph, Integer> oneEntry : qMaps.entrySet()){
			Graph g = oneEntry.getKey();
			...
			if(maxSubs.get(0) == -1)
				...
				if(support >= uppBound)
					...
					queries.add(new TestQuery(g, candidateFetcher.size(), oneEntry.getValue()));
			}
		}
		Collections.sort(queries);
		this.filterAndWrite(queries, queryFile);
	 ...
	 * @throws IOException
	 */
	public ArrayList<TestQuery> loadQueries(String queryFile) throws IOException{

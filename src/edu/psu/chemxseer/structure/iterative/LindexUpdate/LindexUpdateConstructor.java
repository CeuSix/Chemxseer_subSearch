...
public class LindexUpdateConstructor extends LindexConstructor{
	 ...
	 * @throws IOException
	 */
	public static void saveSearcher(LindexUpdateSearcher searcher, String baseName, String indexName) throws IOException{
	 ...
	 * @throws IOException
	 */
	public static LindexUpdateSearcher loadSearcher(String baseName, String indexName) throws IOException{
		...
		while(aLine!=null && aLine.length() != 0){
			...
			LindexTerm thisIndex = new LindexTerm(label,indexTermIndex, frequency);
			
			if(indexTermIndex < featureNum)
				indexTerms[indexTermIndex]  = thisIndex;
			else newTerms[indexTermIndex-featureNum] = thisIndex;
		...
		for(int i = 0; i< tParentsIndex.length; i++){
			...
			else thisTerm = newTerms[i-featureNum];
			
			if(parentId == -1){
				thisTerm.setParent(dummyHead);
				dummyHead.addChildren(thisTerm);
			}
			else if(parentId < featureNum) 
				thisTerm.setParent(indexTerms[parentId]);
			else thisTerm.setParent(newTerms[parentId-featureNum]);
		}
		...
		for(String oneString : tokens){
			...
			else deletedTerms.add(newTerms[termID-featureNum]);
		}
		List<LindexTerm> insertedTerms = new ArrayList<LindexTerm>();
		for(LindexTerm newTerm : newTerms)
			insertedTerms.add(newTerm);
		return new LindexUpdateSearcher(new LindexSearcher(indexTerms, dummyHead), insertedTerms, deletedTerms);
	}

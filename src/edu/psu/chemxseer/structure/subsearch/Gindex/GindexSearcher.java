 ...
 *
 */
public class GindexSearcher implements IndexSearcher{
	...
	/**
	 * Build a Gindex Searcher:
	 * If exhaustSearch == true, then discriminative == null
	 * Else discrimiantive !=null
	 * @param exhaustSearch
	 */
	public GindexSearcher( 
			HashMap<String, Integer> gHash, boolean[] discriminative, int maxEdgeNum,
	...
	private Map<String, Integer> getAllSubgraphs(Graph query) {
		SubgraphGenerator2 subgraphs = new SubgraphGenerator2(query, this.maxEdgeNum);
		//DFSCodeGenerator subgraphs = new DFSCodeGenerator(g, this.maxEdgeNum);
		Integer indexID = -1;
		...
		if(query.getEdgeCount() < this.maxEdgeNum){
			...
			else if(gID!=null && discriminative!=null && discriminative[gID]!=false)
				allIndexSubs.put(gLabels, gID);
		}
		...
		while(true){
			...
			if(label==null) break; // all subgraphs has been 
				...
				if(!exhaustSearch){
					if(indexID==null){
						subgraphs.earlyPruning();// No frequent at all, early pruning
						continue;
					}
					// non frequent 
					...
					// exhaustSearch: either continue searching or adding into allSubs
					if(indexID==null)
						continue;
					else allIndexSubs.put(label, indexID);
				}
			}
		}
		return allIndexSubs;

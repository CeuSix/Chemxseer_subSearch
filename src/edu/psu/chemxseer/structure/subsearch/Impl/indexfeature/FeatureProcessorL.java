...
//	 */
//	public void mineTCFG(IFeatures candidateFeatures, float delta) throws ParseException{
...
//	}
	 ...
	 * @throws IOException 
	 */
	public static PostingFeatures findEdgeOneFeatures(String dbFileName, String edgeFile, String edgePosting, GraphParser gParser) {
		return frequentSubgraphMining(dbFileName, edgeFile, edgePosting,
				(float) 0.1, 1, gParser);
	}
	
	/**
	 * Given the GraphDataset, find all the frequent fragment (all distinct edge 1 features are included)
	 * @param gDBFileName
	 ...
	 * @return
	 */
	public static PostingFeatures frequentSubgraphMining(String gDBFileName, String featureFileName, String postingFileName,
			float minimumFrequency, int maxNonSelectDepth, GraphParser gParser){
		
//		String[] args = {"-minimumFrequencies="+(-minimumFrequency), "-maximumFragmentSize="+maxNonSelectDepth,
//				"-graphFile="+gDBFileName,
//				"-closedFragmentsOnly=flase", "-outputFile=temp", "-parserClass=edu.psu.chemxseer.structure.iso.DFSCoder", 
//				"-serializerClass=edu.psu.chemxseer.structure.iso.CanonicalDFS", "-memoryStatistics=false", "-debug=1"};
		String[] args = {"-minimumFrequencies="+(-minimumFrequency), "-maximumFragmentSize="+maxNonSelectDepth,
				...
				"-serializerClass=edu.psu.chemxseer.structure.iso.CanonicalDFS", "-memoryStatistics=false", "-debug=0"};
		return GSpanMiner_Edge.gSpanMining(args, featureFileName, postingFileName);
	}
	
	/**
	 * Merge all the edge features and subgraphFeatures.
	 * Does not change selected information. 
	 * 
	 ...
	 * @return
	 */
	public static NoPostingFeatures mergeFeatures(NoPostingFeatures subgraphFeatures, 
			NoPostingFeatures<OneFeatureImpl> edgeFeatures){

		List<IOneFeature> allFeatures = new ArrayList<IOneFeature>();
		//1. Add edge features
		int edgeCount = edgeFeatures.getfeatureNum();
		for(int i = 0; i< edgeCount; i++){
			OneFeatureImpl aFeature =  edgeFeatures.getFeature(i);
	 ...
	 * @throws IOException 
	 */
	public static NoPostingFeatures<OneFeatureImpl> removeFeatures(NoPostingFeatures<OneFeatureImpl> subgraphFeatures, 
	...
	throws IOException{
		...
		for(int i = 0; i< subgraphFeatures.getfeatureNum(); i++){
			...
			else oneFeature.setSelected();
		}
		List<OneFeatureImpl> selectedFeatures = subgraphFeatures.getSelectedFeatures();
		return new NoPostingFeatures<OneFeatureImpl>(null, selectedFeatures, false);
	}
}

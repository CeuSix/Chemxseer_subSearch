...
/**
 * This is a implementation of Features_Ext
 * This class is mainly design for the Lindex
 * The major add-on features of this class over the normal NoPostingFeatures classes is 
 * the saving of parents/children relationships
 * @author dayuyuan
 *
 */
public class NoPostingFeatures_Ext<T extends IOneFeature> extends NoPostingFeatures<T>{
	//Features Extension
	private OneFeatureExt[] featuresExt;
	protected boolean subsuperRelationExist;
	
	/**
	 * Given normal Features: construct a "super-sub" graph relationship preserved FeaturesExt
	...
	public NoPostingFeatures_Ext(NoPostingFeatures<T> normalFeatures){
		...
		for(int i = 0; i< normalFeatures.getfeatureNum(); i++)
			this.featuresExt[i] = new OneFeatureExt(normalFeatures.getFeature(i));
	}
	/**
		...
		for(int i = featuresExt.length-1; i >= 0; i--){
			...
			for(int j = i+1; j< featuresExt.length; j++){
				if(offsprings.contains(featuresExt[j]))
					continue;
				boolean isSub = fastSu.isIsomorphic(featuresExt[i].getFeatureGraph(), featuresExt[j].getFeatureGraph());
				if(isSub){
	...
	/**
	 * Reorder the features according to the orders of the featuresExt
	 * Reorder the ID
	 */
	private void reorderUnderlyingFeatures(){
		for(int i = 0; i< this.featuresExt.length; i++){
			features[i] = (T) featuresExt[i].getOriFeature();
	 ...
	 * @param term
	 */
	private void addOffspring(Collection<OneFeatureExt> offsprings,  OneFeatureExt term){

...
public class FeaturePosting implements IFeaturePosting{
	...
	public FeaturePosting(String postingFileName){
		...
		try {
			this.postingFile = new RandomAccessFile(this.postingFileName,"r");
	...
	/**
	 * Read the postings starting from the "featureShift", rewrite it two the postingChannel
	 * and return the new shift
	 * @param postingChannel
	 * @param featureShift
	 * @param featureID
	 * @return
	 */
	public long savePostings(FileChannel postingChannel, long featureShift, int featureID){
		...
		while(start < bytes.length){
			...
			else postbbuf.put(bytes,start,bytes.length-start);
			postbbuf.flip();
			try {
	...
	public static long savePostings(FileChannel postingChannel, int[] postings,int featureID) throws IOException{

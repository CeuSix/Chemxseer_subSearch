...
/**
 * Implementation of a collection of features <type T> (in-memory), no posting files
 * @author dayuyuan
 *
 */
public class NoPostingFeatures<T extends IOneFeature>{
	...
	/**
	 * Load the feature into memory, all the loaded features are of type "OneFeature"
	...
	private void loadFeatures(FeatureFactory factory) throws IOException{
		...
		bin = new BufferedReader(new InputStreamReader(new FileInputStream(featureFileName)));
		String firstLine = bin.readLine();
		if(firstLine==null)
			return;
		features = new IOneFeature[Integer.parseInt(firstLine)];
		//System.out.println(this.features.length);
		// Read one Feature from feature file and Then load it into this.features
		int iter = 0;
	...
	/**
	 * Write the features to the featureFile and Construct a NoPostingFeatures class
	 * @param featureFile
	 * @param features
	 * @param reserveID
	 * @throws IOException 
	 */
	public NoPostingFeatures (String featureFile, List<T> features, boolean reserveID) throws IOException{
		...
		if(featureFile!=null){
			BufferedWriter featureWritter= new BufferedWriter (new FileWriter(featureFile));
	...
	public void saveFeatures(String newFileName) throws IOException {
		...
		else featureWritter = new BufferedWriter(new FileWriter(featureFileName));
	 ...
	 * If NoPostingFeatures is desired, pls use the construcor
	 * @return
	 */
	public List<T> getSelectedFeatures() {
	  ...
	  * If NoPostingFeatures is desired, pls use the constructor
	  * @return
	  */

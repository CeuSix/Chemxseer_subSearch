 ...
 *
 */
public class GraphResultLucenePrefix implements GraphResultPref{
	private Document graphDoc;
	private int docID;
	private IndexSearcherPrefix index;
	 ...
	 * @param graphDoc
	 * @param docID
	 * @param index: Index Storing the Prefix (representation) of the graph.
	 */
	public GraphResultLucenePrefix(Document graphDoc, int docID, IndexSearcherPrefix index) {
		this.graphDoc = graphDoc;
		this.docID = docID;
		this.index = index;
	}
	/**
	 * Return the Prefix Features of this GraphResult:
	 * (1) If there exist the prefix field, then the prefix filed is used
	 * (2) If there is not prefix field, then the subGraphs filed is used
	 */
	...
	public Graph getG() {
		...
		if(prefix != null){
			if(suffix!=null){
				...
				Graph g = MyFactory.getDFSCoder().parse(gString, MyFactory.getGraphFactory());
				g.saveMemory();
				return g;
			}
			else{
				// suffix == null
				return MyFactory.getDFSCoder().parse(prefix, MyFactory.getGraphFactory());
			}
		}
		else if(suffix!=null)
			return MyFactory.getDFSCoder().parse(suffix, MyFactory.getGraphFactory());
		else return null; // both prefix & suffix  == null
	}
	
	/**
	 * Return null if the prefixIndex == -1

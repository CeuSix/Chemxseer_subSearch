 ...
 * But the value set is partitioned into two parts: direct value set & indirect value set
 * @author dayuyuan
 *
 */
public class PostingBuilderLuceneVectorizerPartition implements PostingBuilderLuceneVectorizer {
	private LindexSearcherAdv searcher;
	private GraphParser gParser;
	
	public PostingBuilderLuceneVectorizerPartition(GraphParser gParser, LindexSearcherAdv searcher){
		this.searcher = searcher;
		this.gParser = gParser;
	}
	
	public Document vectorize(int gID, Graph query,  Map<Integer, String> gHash) throws ParseException {
		...
		for(int id: directIDs){
			if(gHash==null){
				...
				gDoc.add(new Field("subGraphs_d", byteString, Field.Store.NO, Field.Index.NOT_ANALYZED));
			}
			else{
				gDoc.add(new Field("subGraphs_d", gHash.get(id), Field.Store.NO, Field.Index.NOT_ANALYZED));
			}
		}
		// for the indirectIDs: subgraphs_i
		IntersectionSet set = new IntersectionSet();
		set.addAll(allIDs);
		set.removeAll(directIDs);
		int[] indirectIDs = set.getItems();
		
		for(int id: indirectIDs){
			if(gHash==null){
				String byteString = new Integer(id).toString();
				gDoc.add(new Field("subGraphs_i", byteString, Field.Store.NO, Field.Index.NOT_ANALYZED));
			}
			else{
				gDoc.add(new Field("subGraphs_i", gHash.get(id), Field.Store.NO, Field.Index.NOT_ANALYZED));
			}
		}
...
//			Field subgraphField = new Field("subGraphs_i", termString, Field.Store.NO, Field.Index.ANALYZED_NO_NORMS);
...
//		}
//		String termString = sBuf.substring(0, sBuf.length()-1); 
//		Field subgraphField = new Field("subGraphs_d", termString, Field.Store.NO, Field.Index.ANALYZED_NO_NORMS);
//		gDoc.add(subgraphField);

...
/**
 * In charge of building the lucene index for postingFetcher. 
 * It need the support of one PostingBuilderLuceneVectorizer. 
 * The vectorizer can be either VectorizerNormal or VectorizerPartition.
 * @author dayuyuan
 *
 */
public class PostingBuilderLucene {
	PostingBuilderLuceneVectorizer vectorizer;
	
	public PostingBuilderLucene(PostingBuilderLuceneVectorizer vectorizer){
		this.vectorizer = vectorizer;
	}
	
	public void buildLuceneIndex(String lucenePath,int maxFieldLength, GraphDatabase gDB,
			Map<Integer, String> gHash) throws CorruptIndexException, LockObtainFailedException, IOException{
		// 1. First Step: construct a index writer
		MaxFieldLength maxL= new MaxFieldLength(maxFieldLength);
		// Here we use NIOFDirectory, preferred class except for running on windows
		Directory dic;
		try {
	...
	private void populateDocsToLucene(int gID, Graph g, IndexWriter indexWriter, Map<Integer, String> gHash) 

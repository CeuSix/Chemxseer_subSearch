...
public class LindexSearcherCompleteEmbedding extends LindexSearcher{
	...
	/**
	 * Given the query, return the maximum subgraph features & their corresponding mappings as well
	 * If an feature is isomorphic to the query, the the map is return as <feature, null>
	 * @param query
	 * @param TimeComponent
	 * @return
	 */
	public HashMap<Integer, int[][]> maxSubgraphs2(Graph query, long[] TimeComponent) {
		...
		for(int i = 0; i< seeds.length; i++){
			...
			if(fastSuExt.isIsomorphic()){
				...
				boolean findPrecise = maximumSubgraphSearch2(fastSuExt, maxSubTermIds, seeds[i]);
				if(findPrecise) {
	...
	private boolean maximumSubgraphSearch2(FastSUCompleteEmbedding fastSu, HashMap<Integer, int[][]> maxSubTerms, LindexTerm oriTerm){
		...
		if(children == null || children.length == 0){
			...
			for(int i = 0;i< children.length; i++){
				...
				if(next.isIsomorphic()){
					...
					boolean success = maximumSubgraphSearch2(next,maxSubTerms, children[i]);
					if(success)
						return true;
				}
			}
		...
		if(extendable == false){
			...
			else maxSubTerms.put(oriTerm.getId(), mapps);
		}
		return false;
	}
}

...
public class SubgraphSearch_LindexPlusBuilder {
	
	public SubgraphSearch_LindexPlus loadIndex(GraphDatabase_OnDisk gDB, String baseName,  GraphParser gSerializer, boolean lucene_in_mem) throws IOException{
		LindexSearcher in_memoryIndex = LindexConstructor.loadSearcher(baseName, SubgraphSearch_LindexPlus.getIn_MemoryIndexName());
		PostingFetcher in_memoryPostings = new PostingFetcherLucene2(baseName + SubgraphSearch_LindexPlus.getLuceneName(), gDB.getTotalNum(), gSerializer, lucene_in_mem);
		...
		return new SubgraphSearch_LindexPlus(new LindexSearcherAdv(in_memoryIndex), in_memoryPostings,on_diskPostings, new VerifierISO(), baseName);
	}
	
	public SubgraphSearch_LindexPlus buildIndex (NoPostingFeatures_Ext features, 
			NoPostingFeatures onDiskFeatures,GraphDatabase_OnDisk gDB, String baseName
			,  GraphParser gSerializer, boolean lucene_in_mem) throws IOException, ParseException{
		...
		PostingFetcher on_diskFetcher = this.buildOnDiskIndex(in_memoryIndex, onDiskFeatures, baseName, gDB, gSerializer, lucene_in_mem);
		// 3rd step: return
		PostingFetcher posting = new PostingFetcherLucene2(baseName + SubgraphSearch_LindexPlus.getLuceneName(), gDB.getTotalNum(), gSerializer,  lucene_in_mem);
		return new SubgraphSearch_LindexPlus(in_memoryIndex, posting,on_diskFetcher, new VerifierISO(), baseName);
	}
	
	protected PostingFetcher buildOnDiskIndex(LindexSearcher lindex, NoPostingFeatures features, String baseName,
			GraphDatabase_OnDisk gDB,  GraphParser gSerializer, boolean lucene_in_mem) throws IOException, ParseException{
		long time1 = System.currentTimeMillis();
		
		List<IOneFeature>[] featureBelongs = new ArrayList[lindex.getFeatureCount()];
		// First step: find the maximum subgraph (only one) index terms for each feature in allFeatures
		for(int i = 0; i< features.getfeatureNum(); i++){
			...
			List<Integer> maxSubs = lindex.maxSubgraphs(theFeature.getFeatureGraph(), TimeComponent);
			
			// skip if theFeature is already in the Lindex
			if(maxSubs!=null && maxSubs.get(0)==-1) 
				continue;
			else if(maxSubs == null || maxSubs.size() == 0){
				System.out.println("It is so Wired in LindexCompleteAdvance: constructOnDisk: no subgraphs");
				return null;
			}
		...
		// After above step: each in-memory index term is associated with a set of on-disk features:
		...
		for(int i = 0; i< in_memoryFeatureCount; i++){			
			...
			if(featureBelongs[i] == null || featureBelongs[i].size() == 0)
				continue;
			IOneFeature[] on_diskFeaturesTemp = new IOneFeature[featureBelongs[i].size()];
			for(int w = 0; w < on_diskFeaturesTemp.length; w++){
				featureBelongs[i].get(w).setFeatureId(w);
				on_diskFeaturesTemp[w] = featureBelongs[i].get(w); // change of feature 'ID'
			}
			NoPostingFeatures on_diskFeatures = new NoPostingFeatures(on_diskFeaturesTemp);
			NoPostingFeatures_Ext inputFeatures = new NoPostingFeatures_Ext(on_diskFeatures);
			try {
				inputFeatures.mineSubSuperRelation(); // may change of feature 'ID' again
			} catch (ParseException e) {	
				e.printStackTrace();
			}	
			for(int w = 0; w < inputFeatures.getfeatureNum(); w++){
				featuresIndexString.put(inputFeatures.getFeature(w).getDFSCode(), i + "_" + w);
			}
			// B: Start Building OnDisk Lindex & save them
			LindexSearcher cons = LindexConstructor.constructOnDisk(inputFeatures, lindex, i);
			LindexConstructor.saveSearcher(cons,baseName, SubgraphSearch_LindexPlus.getOnDiskIndexName(i));
		}
		
		long time2 = System.currentTimeMillis();
		System.out.println("4. Build Lindexes on-disk: " +(time2-time1));
		//C: build lucene index
		NoPostingFeatures_Ext luceneFeatures = new NoPostingFeatures_Ext(features);
...
//		features.clearSubSuperRelation();
		...
		// Because the minSubSuperRelation involves sort of all the featues, thus changes the feature id
		for(int i = 0; i< luceneFeatures.getfeatureNum(); i++){
			String indexString = featuresIndexString.get(luceneFeatures.getFeature(i).getDFSCode());
			gHash.put(i, indexString);
			luceneFeatures.getFeature(i).setFeatureId(i); // change of feature ID
		}
		LindexSearcher consForLuceneLindex = LindexConstructor.construct(luceneFeatures);

		PostingBuilderLucene postBuilder = new PostingBuilderLucene(
				new PostingBuilderLuceneVectorizerNormal(gSerializer, consForLuceneLindex));
		postBuilder.buildLuceneIndex(baseName + SubgraphSearch_LindexPlus.getOnDiskLuceneName(), consForLuceneLindex.getFeatureCount(), gDB, gHash);
		long time3 = System.currentTimeMillis();
		System.out.println("5. Build lucene for Lindexes on-disk: " + (time3-time2));
		
		return new PostingFetcherLucene(baseName + SubgraphSearch_LindexPlus.getOnDiskLuceneName(), gDB.getTotalNum(), gSerializer, lucene_in_mem);
	}
}

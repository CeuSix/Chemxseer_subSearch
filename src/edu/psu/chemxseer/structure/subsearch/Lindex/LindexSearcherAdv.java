...
public class LindexSearcherAdv extends LindexSearcher{
	...
	public LindexSearcherAdv(LindexSearcher searcher){
		...
		for(int i = 0; i< this.indexTerms.length; i++){
			Graph g = MyFactory.getDFSCoder().parse(this.getTermFullLabel(indexTerms[i]), MyFactory.getGraphFactory());
			termLabels.add(MyFactory.getDFSCoder().serialize(g));
		}
	}
	/**
	 * Given all the subGraph features contained in the query, find and return the set of features
	 * directly connected with the query (q is in the direct value set of f)
	 * @param query
	 * @param allSubIds
	...
	public Set<Integer> getDirectFeatures(Graph query, List<Integer> allSubIds){
		...
		for(int i = 0; i< allSubIds.size(); i++){
			...
			if(query.getEdgeCount() <= indexTermGraph.getEdgeCount() + 1 && query.getNodeCount() <= 
				indexTermGraph.getNodeCount() + 1)
				directFeatures.add(allSubIds.get(i)); // Verfied without the need of direct test
			else if(this.directTest(indexTermGraph, query, this.indexTerms[allSubIds.get(i)], this.getMaxEdgeNum(), termLabels))
	...
	private int getMaxEdgeNum(){
		if(maxEdgeNum != -1)
			...
			for(int i =0; i< this.indexTerms.length; i++){
				if(this.indexTerms[i].getChildren()==null || this.indexTerms[i].getChildren().length == 0){
					// For each leaf node
	...
	public boolean directTest(Graph termGraph, Graph targetGraph, LindexTerm theTerm, 
			int maxEdge, Set<String> termLabels){
		
		LindexTerm[] children = theTerm.getChildren();
		if(children == null || children.length == 0)
			return true; // the big graph in fastSu is direct to theTerm
	    // Step to take: 
		// 1. First find the complete mapping between termGraph and the target graph
		FastSUExt fastSu = new FastSUExt();
		FastSUState fastSuState = fastSu.completeEmbedding(termGraph, targetGraph);
		if(fastSu.getMapNumber() == 0){
			System.out.println("YDY: It is wired in direct Test");
		}
		else{
			int[][] mappings = fastSu.getMappings();
			// 2. Second starting from each mapping, grows the whole set of subgraphs that
			...
			for(int i = 0; i< mappings.length; i++)
			{
				...
				SubgraphGenerator3 subgraphsGenerator = new SubgraphGenerator3(fastSuState, maxEdge+2);
				...
				while(subString!=null)
				{
					...
					else if(edgeNodeNum[0] > maxEdge || edgeNodeNum[0] == targetGraph.getEdgeCount()-1 )
							return true;  // there is a path from termGraph to this subString
					// else keep on growing
					subString = subgraphsGenerator.nextSubgraph(edgeNodeNum);
				}
			}
		}
		// Else: every subgraphs of TargetGraph is earlyPruned, thus return false
		return false;
	}
	
	private Graph getTermGraph(int termId){
		int[][] fullLabel = this.getTermFullLabel(this.indexTerms[termId]);
		return MyFactory.getDFSCoder().parse(fullLabel, MyFactory.getGraphFactory());
	}

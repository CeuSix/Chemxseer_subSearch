...
/**
 * size n edge entry records all graphs that are of size n
 * size n edgeEntry is an arraylist of edgeArraies
 * @author dayuyuan
 *
 */
public class IN_FGindexEdgeEntry {
	...
	public IN_FGindexEdgeEntry(int graphSize, int index, IN_FGindexEdgeArray[] edgeArrays){
		this.graphSize = graphSize;
		this.indexInEdges = index;
	...
	public IN_FGindexEdgeArray[] getEdgeArrays() {
		IN_FGindexEdgeArray[] results = new IN_FGindexEdgeArray[edgeArrays.size()];
	 ...
	 * If such EdgeArray do not exist, first create such a sizeM
	 * EdgeArray, and then return it
	 * @param freqM
	 * @return
	 */
	public IN_FGindexEdgeArray getAndSetFreqMEdgeArray(int freqM){
		for(int i = 0; i< this.edgeArrays.size(); i++)
		{
			if(this.edgeArrays.get(i).getArrayFrequency() == freqM)
				...
				if(this.edgeArrays.get(i).getArrayFrequency() > freqM)
				{
					IN_FGindexEdgeArray edgeArray = new IN_FGindexEdgeArray(freqM, i);
		...
		IN_FGindexEdgeArray edgeArray = new IN_FGindexEdgeArray(freqM, numofArrays);
	...
	/**
	 * Given the minimum frequency
	 * Find the minimal edgearray that has frequency greater or equal to minfreq
	 * @param minfreq
	 * @return
	 */
	...
	/**
	 * Given the maximum frequency
	 * Find the maximal edgearray that has frequency smaller than maxfreq
	 * @param maxfreq
	 * @return
	 */
	...
	public int getMaximumFrequency(){
		IN_FGindexEdgeArray last = this.edgeArrays.get(this.edgeArrays.size()-1);

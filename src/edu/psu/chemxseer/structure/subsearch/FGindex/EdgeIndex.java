...
import edu.psu.chemxseer.structure.subsearch.Impl.GraphResultNormal;
...
/**
 * Edge Index should be a index, composed of indexSearcher (lookup), postingFetcher and verifier. 
 * 
 * However, due to the previous implementation, those three parts are hardcooded and wired together
 * Thus, a warper is designed instead. 
 * @author duy113
 *
 */
public class EdgeIndex{
	...
	private GraphDatabase gDB; // only for load of infrequent graphs
	/**
	 * For index loading: In_memory
	 * @param gDB
	 */
	public EdgeIndex(GraphDatabase gDB){
		this.gDB = gDB;
	...
	/**
	 * iven a Graph querys, search for all distinct edges of this graph
	 * save them in pair: <FGindexEdge, inner frequency of this edge on graph>
	 * In_FGindexEdge is an edge
	 * Integer it the frequency of the edge
	 * @param query
	 * @return
	 */
	public Map<IN_FGindexEdge, Integer> getAllEdges(Graph query){
		...
		for(int edgeIndex = 0; edgeIndex < edgeCount; ++edgeIndex){
			...
			IN_FGindexEdge fgEdge = new IN_FGindexEdge(query.getNodeLabel(query.getNodeA(edge)), 
					query.getNodeLabel(query.getNodeB(edge)), query.getEdgeLabel(edge));
	 ...
	 * @param g
	 * @param timeComponent[2]
	 * @return
	 */
	public  GraphFetcher getInfrequentEdgeCandidates(Graph g, long[] timeComponent){
		long start  = System.currentTimeMillis();
		Set<Entry<IN_FGindexEdge, Integer>> edgesofG = this.getAllEdges(g).entrySet();
		int graphSize = g.getEdgeCount();
		IntersectionSet set = new IntersectionSet();
		boolean firstTime = true;
		for(Iterator<Entry<IN_FGindexEdge, Integer>> iter = edgesofG.iterator();iter.hasNext();){
			// For each distinct edge in this graph
			Entry<IN_FGindexEdge, Integer> currentEntry= iter.next();
			// Add graph with id = graphIndex into the triple(graphSizeN, frequencyM, id)
			// of a edgeEntries currentEndgeEntries associated with this edgeSet
			IN_FGindexEdgeEntries currentEdgeEntries = this.edgeIndex.get(currentEntry.getKey());
			if(currentEdgeEntries != null && !currentEdgeEntries.isFrequent())
			{
				if(firstTime){
					int[] result1 =currentEdgeEntries.getGraphsWithMinSizeMinFreq(graphSize, currentEntry.getValue()); 
					set.addAll(result1);
					firstTime = false;
				}
				else {
					int[] result2 = currentEdgeEntries.getGraphsWithMinSizeMinFreq(graphSize, currentEntry.getValue());
					set.retainAll(result2);
				}
			}
		}
	...
	/************Construction*******************************/
	/**
	 * Add a not Frequent Graph [database graph]
	 * into infrequent edge index
	 * @param g
	 * @return
	 */
	private boolean insertGraph(Graph g, int gid){
		...
		for(Iterator<Entry<IN_FGindexEdge, Integer>> iter = edgesofG.iterator();iter.hasNext();){
			...
			if(currentEdgeEntries.isFrequent())
				...
				currentEdgeEntries.addGraph(g.getEdgeCount(), currentEntry.getValue(), gid);
		}
		return true;
	}
	private void sortIndex() {
		Set<Entry<IN_FGindexEdge, IN_FGindexEdgeEntries>> edges = this.edgeIndex.entrySet();
		for(Iterator <Entry<IN_FGindexEdge, IN_FGindexEdgeEntries>> eIt = edges.iterator(); eIt.hasNext();){
	...
	/************************Index Saving ******************************************/
	/**
	 * EdgeIndex: A, B, e(A, B), frequent/infrequent, long shift
	 * 2. EdgeIndexFile:
	 * Each line represent each EdgeEntries
	 * <Entry1><Entry2><Entry3>
	 * Entry1 = [graph size][edgeArray1][edgeArray2][edgeArray3]
	 * edgeArray1 = arraySize, graphIDshift
	 * 3. GraphIdshift
	 * id, id, id, id \n
	 * @param index
	 * @param indexWriter
	 ...
	 * @throws IOException 
	 */
	public void writeEdges(BufferedWriter indexWriter, FileChannel edgeWriter, FileChannel graphsWriter) throws IOException{
		Set<Entry<IN_FGindexEdge, IN_FGindexEdgeEntries>> edgeSet = edgeIndex.entrySet();
		for(Iterator<Entry<IN_FGindexEdge, IN_FGindexEdgeEntries>> it = edgeSet.iterator(); it.hasNext();){
			Entry<IN_FGindexEdge, IN_FGindexEdgeEntries> currentEntry = it.next();
			...
			buf.append(',');
			// The write the shift
			long shift = wirteEdgeEntry(currentEntry.getValue(), edgeWriter, graphsWriter);
			buf.append(shift);
			buf.append('\n');
	 ...
	 * @throws IOException 
	 */
	private long wirteEdgeEntry(IN_FGindexEdgeEntries entries, FileChannel edgeWriter, FileChannel graphsWriter) throws IOException{
		...
		for(int i = 0; i< entryArray.length; i++){
			...
			for(int j = 0; j < edgeArrays.length; j++){
				...
				buf.append(',');
				long graphIDshift = writeGraphIDs(edgeArrays[j].getGraphIDs(), graphsWriter);
		...
		while(start < bytes.length){
			...
			else bbuf.put(bytes,start,bytes.length-start);
			bbuf.flip();
	...
	private long writeGraphIDs(int[] ids, FileChannel graphsWriter) throws IOException{
	...
	/*********************Index Loading 
	 * @throws IOException ***********************/
	public void loadIndex(BufferedReader indexReader, RandomAccessFile edgeReader, RandomAccessFile graphReader) throws IOException{
		...
		while(edgeLine!=null){
			...
			if(temp2 == 0)
				isFrequent = false;
			IN_FGindexEdge oneEdge = new IN_FGindexEdge(Integer.parseInt(edgeTokens[0]), 
					Integer.parseInt(edgeTokens[1]), Integer.parseInt(edgeTokens[2]));
			long entriesShift = Long.parseLong(edgeTokens[4]);
			IN_FGindexEdgeEntries oneEntries = loadEntries(entriesShift, edgeReader, graphReader, isFrequent);
	 ...
	 * @throws IOException 
	 */
	private IN_FGindexEdgeEntries loadEntries(long shift, RandomAccessFile edgeReader, 
			RandomAccessFile graphReader, boolean isFrequent) throws IOException{
		...
		for(int i = 0; i< edgeEntries.length;i++){
			...
			String entryLine = entryTokens[i].substring(1, entryTokens[i].length()-1);
			...
			for(int j = 0; j < edgeArrays.length; j++){
				...
				edgeArrays[j] = new IN_FGindexEdgeArray(arraySize, graphIds, true, j);
			}
			edgeEntries[i] = new IN_FGindexEdgeEntry(graphSize, i,edgeArrays);
		}
		return new IN_FGindexEdgeEntries(edgeEntries, isFrequent);
	}
	private int[] loadGraphIDs(long shift, RandomAccessFile graphReader) throws IOException{
	...
	public boolean containInfrequentEdges(Graph theGraph) {
		long start  = System.currentTimeMillis();
		Set<Entry<IN_FGindexEdge, Integer>> edgesofG = this.getAllEdges(theGraph).entrySet();
		int graphSize = theGraph.getEdgeCount();
		for(Iterator<Entry<IN_FGindexEdge, Integer>> iter = edgesofG.iterator();iter.hasNext();){
			// For each distinct edge in this graph
			Entry<IN_FGindexEdge, Integer> currentEntry= iter.next();
			// Add graph with id = graphIndex into the triple(graphSizeN, frequencyM, id)

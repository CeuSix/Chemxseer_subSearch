...
public class IN_FGindexEdgeEntries {
	...
	/**
	 * Return the calculated memory consumption of this class
	 * Treat ArrayList as an array
	 * No 20 bytes memory for an empty object
	 * @return
	 */
	public long getMemoryConsumption() {
		// boolean frequent
		long memoryConsumption = 1;
		// The pointer at each entry of the array takes 8 byte, omit the size of the array (20 byte)
	...
	/**
	 * Return all graphs in this graph entries, that are of size = graphSize
	 * and its edge frequency >= minimumFrequency
	 * Union
	 * @param graphSize
	...
	public int[] getGraphsWithMinimumFreq(int graphSize, int minimumFrequency){
		...
		if(oneEntry == null)
			...
			IN_FGindexEdgeArray oneArray = oneEntry.getEdgeArrayMinFreq(minimumFrequency);
			
			if(oneArray == null)
				return null;
			IntersectionSet set = new IntersectionSet();
			set.addAll(oneArray.getGraphIDs());
			IN_FGindexEdgeArray biggerArray = oneEntry.getBiggerEdgeArray(oneArray);
			while(biggerArray!=null){
				if(biggerArray.getGraphIDs()!=null)
					set.addAll(biggerArray.getGraphIDs());
	...
	/**
	 * Return all graphs in this graph entries, that are of size = graphSize
	 * and its edge frequency <=- maximumFrequency
	 * @param graphSize
	...
	public int[] getGraphsWithMaximumFreq(int graphSize, int maximumFrequency) {
		...
		if(oneEntry == null)
			...
			IN_FGindexEdgeArray oneArray = oneEntry.getEdgeArrayMaxFreq(maximumFrequency);
			if(oneArray == null)
				return null;
			
			IntersectionSet set = new IntersectionSet();
			set.addAll(oneArray.getGraphIDs());
			IN_FGindexEdgeArray smallerArray = oneEntry.getSmallerEdgeArray(oneArray);
			while(smallerArray!=null){
				if(smallerArray.getGraphIDs()!=null)
					set.addAll(smallerArray.getGraphIDs());
	...
	/**
	 * Return all graphs in this graph entries, that are of size = graphSize
	 * and its edge frequency = frequency
	 * @param graphSize
	 * @param frequency
	 * @return
	 */
	...
	public int[] getGraphsWithMinSizeMinFreq(int minGraphSize, int minFrequency){
		...
		for(int i = lastIndex; i >=0; i--){
			...
			if(graphSize < minGraphSize)
				break;
			int[] graphsOfFixSizeMinFrequency = this.getGraphsWithMinimumFreq(graphSize, minFrequency);
			if(graphsOfFixSizeMinFrequency == null)
				continue;
			set.addAll(graphsOfFixSizeMinFrequency);
		}
		return set.getItems();
	}
	...
	public IN_FGindexEdgeEntry[] getEdgeEntries() {
		if(edgeEntries == null)
			return null;
		IN_FGindexEdgeEntry[] results = new IN_FGindexEdgeEntry[this.edgeEntries.size()];
	...
	public void setEdgeEntries(IN_FGindexEdgeEntry[] edgeEntries, boolean sorted) {
		...
		if(this.edgeEntries == null)
			this.edgeEntries = new ArrayList<IN_FGindexEdgeEntry>(edgeEntries.length);
		else this.edgeEntries.clear();
	...
	/**
	 * given an sizeN
	 * Find a EdgeEntry of sizeN in this EdgeEntries
	 * If such EdgeEntry do not exist, first create such a sizeN
	 * EdgeEntry, and then return it
	 * @param sizeN
	 * @return
	 */
	public IN_FGindexEdgeEntry getAndSetSizeNEdgeEntry(int sizeN){
		for(int i = 0; i< this.edgeEntries.size(); i++)
		{
			if(this.edgeEntries.get(i).getEntrySize() == sizeN)
				...
				if(this.edgeEntries.get(i).getEntrySize() > sizeN)
				{
					IN_FGindexEdgeEntry edgeEntry = new IN_FGindexEdgeEntry(sizeN, i);
	...
	/**
	 * Return the edgeEntry if find it
	 * Return null if dose not exist
	 * @param sizeN
	 * @return
	 */
	...
	/**
	 * Given the current edgeEntry, get the edgeEntry that precede this edgeEntry
	 * @param edgeEntry
	 * @return
	 */
	...
	/**
	 * Given the current edgeEntry, get the edgeEntry that succeed this edgeEntry.
	 * @param edgeEntry
	 * @return
	 */
	...
	/**
	 * Add this graph with graphIndex into an EdgeEntry of size edgeNum
	 * and an EngeArray of size frequency
	 * @param edgeNum
	 * @param frequency
	 * @param graphIndex
	 */
	public void addGraph(int edgeNum, int frequency, int graphIndex) {
		...
		IN_FGindexEdgeArray edgeArray = edgeEntry.getAndSetFreqMEdgeArray(frequency);

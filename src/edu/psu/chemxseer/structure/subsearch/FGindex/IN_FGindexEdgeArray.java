...
/**
 * Size m arrays is a collection of graphs whose
 * edgenumber = n, with m appearance of certain distinct edge
 * pay attention: those IDs in each EdgeArray is sorted
 * @author dayuyuan
 *
 */
public class IN_FGindexEdgeArray {
	...
	public IN_FGindexEdgeArray(int frequency, int[] graphs, boolean sorted, int indexInEdgeEntry){
	...
	public void addGraphID(int graphId){
		...
		if(this.sizeOfGraphIDs < this.graphIDs.length)
		{
			...
			// For test only
			System.out.println("It is wired in addGraphsID: " + sizeOfGraphIDs + " : " + graphIDs.length);
		}
		this.sorted = false;
	}

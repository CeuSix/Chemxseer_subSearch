...
public class FGindexConstructor{
	/**
	 * on-disk constructor
	 * Does not populate the infrequent database graph
	 * @param features
	 */
	public static FGindexSearcher constructOnDisk(Collection<IOneFeature> features){
	 ...
	 * @return
	 */
	public static FGindexSearcher constructInMem(Collection<IOneFeature> features, GraphDatabase_OnDisk gDB){
		String[] graphArray = new String[features.size()];
	...
	/**
	 * Insert one Feature into the index, update the edgeIndex & also the graphArray
	 * @param oneFeature
	 * @param edgeIndex
	 * @return
	 */
	private static void addFeature(String[] graphArray, IOneFeature oneFeature, 
		...
		Set<Entry<IN_FGindexEdge, Integer>> edgesofG = edgeIndex.getAllEdges(g).entrySet();
		...
		for(Iterator<Entry<IN_FGindexEdge, Integer>> iter = edgesofG.iterator();iter.hasNext();){
			...
			IN_FGindexEdgeEntries currentEdgeEntries = edgeIndex.get(currentEntry.getKey());
			...
			currentEdgeEntries.addGraph(oneFeature.getFeatureGraph().getEdgeCount(), 
					currentEntry.getValue(), ID);
		}
	}

	public static void saveSearcher(FGindexSearcher searcher, String baseName, String indexFileName) throws IOException{
		BufferedWriter indexWriter = new BufferedWriter(new FileWriter(new File(baseName,indexFileName)));
		FileChannel edgeIndexWriter = new FileOutputStream(baseName + indexFileName + "_edge").getChannel();
		FileChannel graphIDsWriter = new FileOutputStream(baseName + indexFileName + "_db").getChannel();
		// a. Write graphs 
		writeFeatures(searcher, indexWriter);
		// b. Write edges index
		searcher.edgeIndex.writeEdges(indexWriter, edgeIndexWriter, graphIDsWriter);
		graphIDsWriter.close();
		edgeIndexWriter.close();
		indexWriter.close();
	}

	private static void writeFeatures(FGindexSearcher searcher, BufferedWriter indexWriter){
	...
	/* load the FGindex file from disk, into 3 file: 
	 * 1. IndexFile
	 * First Line: size
	 * StartFome the second Line: id, graphs in graphArray [smiles]
	 * EdgeIndex: A, B, e(A, B),isSelected, long shift
	 * 2. EdgeIndexFile:
	 ...
	 * @throws IOException 
	 */
	public static FGindexSearcher loadSearcher(String baseName, String indexFileName, GraphDatabase gDB) throws IOException{
		BufferedReader indexReader = new BufferedReader(new FileReader(new File(baseName, indexFileName)));
	 ...
	 * @throws IOException
	 */
	private static void loadGraphs(String[] graphArray, BufferedReader indexReader) throws IOException{

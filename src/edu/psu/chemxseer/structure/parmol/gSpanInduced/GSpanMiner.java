...
public class GSpanMiner extends AbstractMiner {
	...
	protected void startRealMining() {
		...
		DataBaseModified gs = new DataBaseModified(m_graphs, dayuFrequency, m_frequentSubgraphs, factory);
	...
	private float[] subgraph_Mining(DFSCode code) {
		...
		for (; it.hasNext();) {
			...
			if (next.toFragment().getFragment().getEdgeCount() < this.minMustSelectSize
					||(next.isFrequent(m_settings.minimumClassFrequencies)
	 ...
	 * 
	 * @param args
	 * @param featureFileName: save all the features
	 * @param coverFileName: save the induced postings only 
	 * @param minMustSelect
	 * @param maxMustNonSelectSize
	 * @return
	 */
	public static PostingFeatures gSpanMining(String[] args, String featureFileName,
			String coverFileName, int minMustSelect, int maxMustNonSelectSize)
	{
		//1. Parse the input
		Settings s =  parseInput( args);
		//2. Mining
		GSpanMiner m = doFrequentFeatureMining(s, minMustSelect, maxMustNonSelectSize);
		//3. return
		try {
	 ...
	 * @return
	 */
	private static GSpanMiner  doFrequentFeatureMining(Settings s, int minMustSelect,  int maxMustNonSelectSize) {
		if (s.directedSearch) {
    ...
    private static PostingFeatures getFeatures(GSpanMiner m, Settings s, String featureFileName, String coverFileName) 
	throws IOException{

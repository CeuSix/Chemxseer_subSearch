...
//    }
	/**
	 * 
	 * An extension version of the searchChildren in DFSCode
	 * The major change is the involvement of the "coverEdges"
	 * Besides adding the support for each "code", we also find the
	 * cover of each "code". 
	 * By "cover", we mean graphs has the code as an edge induced subgraph
     * searches for children of the embedding in the database graph
     * @param dataBaseGraph 
     * @param ackNodes maps embedding node to real node
     * @param usedNodes maps real node to embedding node (lastNode+1 if unused)
    ...
    private void searchCoverSet(GSpanGraph databaseGraph, 
    		int[] ackNodes, int[] usedNodes, int[] usedEdges){
    ...
    /**
     * Test wehter current code is an edge reduced subgraph of the database Graph
     * @param dataBaseGraph 
     * @param ackNodes maps embedding node to real node: ackNodes[codeNodeID] = graphNodeID
     * @param usedNodes maps real node to embedding node (lastNode+1 if unused)
     * 		  usedNodes[graphNodeID] = codeNodeID
     * @param usedEdges tells if real edge is unused (Graph.NO_EDGE) or used (else), used or unused
     * @return
     */
    private boolean isEdgeReduced(GSpanGraph databaseGraph, 
    		int[] ackNodes, int[] usedNodes, int[] usedEdges){
    	...
    	for(int i = 0; i< usedNodes.length; i++){
    		...
    		for(int j = 0; j< degree; j++){
    			...
    			// then the code is not an edge induced subgraph of the database graph
...
//      --
    ...
    public FrequentFragment toFragment(){
        ...
        return new FrequentFragment((Graph) graph.clone(),realSet, realSet2 ,myset.getFreq());
    }

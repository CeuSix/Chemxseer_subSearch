...
/**
 * @deprecated
 * This class mainly focused on finding the whole set of subgraph given a query graph g
 * We use reverse search tactics:
 * When subgraph g is empty, it possible extension are all single nodes in query q.
 * When subgraph g is a tree, it can be both edge & node + edge extended
 * When subgraph g is a graph, it can only be edge extended
 * NodeExtension: add in a node(and the edge connecting this new node and already in graph nodes), 
 * 				and after adding i, this node should have the largest index 
 * 				among all leaf nodes of this extended graph. 
 * EdgeExtension: add in a edge (connecting two already in graph nodes), 
 * 				and after adding e, this edge should have the largest index
 * 				among all edges that in the same cycle of e. 
 * @author dayu yuan
 *
 */
public class SubgraphGenerator {
	...
	// nodeVisitedOrder[i] = -1 / currentDepth of class when the ith node is visited
	...
	public SubgraphGenerator(Graph g, int maximumDepth, CanonicalDFS parser, GraphFactory factory){
		this.g = g;
	...
	/**
	 * Return true if a subgraph extension is applied
	 * Return false if can't not find a reasonable extension
	 * @return
	 */
	private boolean depthFirstNext(){
		...
		if(this.visitedNodeNum-1 == this.visitedEdgeNum){// tree
			if(!this.candidateNodes.get(currentDepth).isEmpty())// Can grow node
			{
				canGrow = true;
				if(currentDepth !=0)
					extendNodeEdge(this.candidateNodes.get(currentDepth).poll(),
						this.candidateNodeParent.get(currentDepth).poll());
				else extendNodeEdge(this.candidateNodes.get(currentDepth).poll(),-1);
			}
	...
	private boolean extendNodeEdge(int nextNode, int nextNodeParent){
		...
		this.currentDepth ++;
		int nextNodeinSubgraph = this.subgraph.addNodeAndEdge(nextNodeParent, connectivity[nextNode][nextNode],edgeLabel[edge]);
		...
		for(int i = 0; i< vertices[nextNode].length;i++){
			...
			{	
				...
				if(tempEdge!=edge && tempEdge > maxEdge(nextNode, anotherNode))
	...
	private int maxEdge(int nodeA, int nodeB){
		...
		while(!queue.isEmpty()){
			...
			for(int i = 0; i< vertices[node].length; i++){
				...
				if(temp[otherNode]==-1)// this otherNode has not been visited before
				{
					tempParent[otherNode]=node;// Assign node as the parent of otherNode
	...
	private boolean extendEdge (int edge){
		...
		this.subgraph.addEdge(nodeMapping[nodeA], nodeMapping[nodeB], edgeLabel[edge]);
	...
	private boolean traceBackNode(){
		...
		// recover candidateEdge (because of the involvement of theNode, several loopEdge is added into candidateEdge)
		while(true){
			int lastAddedEdge = this.candidateEdge.peek();
			if(g.getNodeA(lastAddedEdge)==theNode || g.getNodeB(lastAddedEdge)==theNode)
	...
	private boolean traceBackEdge(){
		...
		int theEdgeinSubgraph = this.subgraph.getEdge(nodeMapping[nodeA], nodeMapping[nodeB]);

...
/**
 * An extension of SubgraphGenerator2
 * The SubgraphGenerator3 start subgraphs generation from a small subgraph
 * And generate all super graphs to maximumDepth size
 * @author duy113
 *
 */
public class SubgraphGenerator3 {
	/* Inner representation of query graph */
	private int[][] vertices; // Linked List representation of g, vertices[nodeI][j]= nodeJ
	private int[][] connectivity; 
	// Matrix representation of g, connectivity[nodeI][nodeJ] = edge(nodeI, nodeJ);
	...
	/**
	 * Given a small graph represented as: connectivityS and verticesS
	 * and a big graph represented as: connectivityB and connectivityB
	 * The subgraphgenerator3 can generate all subgraphs of the big graph
	 * starting from the smaller graph and with the constrain of maximumDepth
	 * verticesS[i] in the smaller graph has the same index as verticesB[i]
	 * in the larger graph 
	 * if maximuDepth = 10, then the maximum subgraph returned has edge  = 9
	...
	public SubgraphGenerator3(FastSUState state, int maximumDepth){
		...
		SubgraphGenerator3(state.getSmallMatrix(), 
				state.getSmallLinkList(), state.getBigMatrix(), state.getBigLinkList(),maximumDepth);
	}
	public SubgraphGenerator3(int[][] connectivityS, int[][] verticesS,
			int[][] connectivityB, int[][] verticesB, int maximumDepth){
		SubgraphGenerator3(connectivityS, verticesS, connectivityB, verticesB, maximumDepth);
	}
	
	public void SubgraphGenerator3(int[][] connectivityS, int[][] verticesS,
			int[][] connectivityB, int[][] verticesB, int maximumDepth) {
		internalGraphInitialization(connectivityS, verticesS, connectivityB, verticesB);
	 ...
	 * @param g
	 */
	private void internalGraphInitialization(int[][] connectivityS, int[][] verticesS,
			int[][] connectivityB, int[][] verticesB){
		...
		for(int i = 0; i< nodeCountB; i++){
			...
			for(int jIndex = 0; jIndex< vertices[i].length;jIndex++){
				...
				// Because of symmetry, we skip the assignment of edges array when i > anotherNode
				if(i < jNode){
					// both node i and node j are in smaller graph, edge(i, j) in the smaller graph
	...
	private void recordInitialization(int[][] verticesS, int maxDepth){
		...
		else this.maximumSubgraphDepth = edgeLabel.length;
	...
	public String nextSubgraph(int[] edgeNodeNum){
		...
		//Write the subgraph in a matrix representation: subConnectivity, & subEdges
		int[] subSuperNodeMap = new int[vertices.length];// subSuperEdgeMap[superNode] = subNode
			...
			if(subgraphNodesDegree[superNodeI]!=-1){
				...
				for(int j = 0; j< subgraphNodesDegree[superNodeI]; j++){
					...
					if(superNodeJ < superNodeI)
						continue; // make sure of the symmetric: each edge is only visited once
					...
					// make sure of the symmetric: both connectivity[i][j] and connectivity[j][i] has to be assigned
					++subEdge;
				}
			}
		}
	...
	public Graph nextSubgraphG(){
		...
		for(int i = 0; i< candidateValidIndex;i++){
			...
			if(oneLeg.isNodeOnlyExtension()){
				...
				int node = g.addNodeAndEdge(nodeMapping[oldNode],connectivity[newNode][newNode], 
				...
				g.addEdge(nodeMapping[nodeA], nodeMapping[nodeB], edgeLabel[connectivity[nodeA][nodeB]]);
			}
		}
		g.saveMemory();
		return g;
	...
	public boolean earlyPruning(){
		if(candidateValidIndex == this.minimumSubgraphDepth+1) return false;
		else restoreSubgraph();
		return true;
	}
	/**
	 * Before depth first search, subgraph is a empty set
	 * possible extension is a node extension, changing empty
	 * subgraph to a one node subgraph. 
	 */
	private void seeds(){
	...
	/**
	 * The process of subgraphs generation is actually a depth first search of a subgraph 
	 * generation tree
	 * In each step, one node + edge or only one edge is added, generating a new subgraph,
	 * and produces a set of possible candidates.  
	 */
	private boolean depthFirstSearch(){
		// this candidates not actually valid to maximumSubgraphDepth
		// Only grows candidatValidIndex to maximumSubgraphDepth because subgraph reaches the upper bound of its size
		...
		if(candidates[candidateValidIndex].isEmpty()){
			if(candidateValidIndex == this.minimumSubgraphDepth+1) return false;// no further track back
			else{
				restoreSubgraph();
				return depthFirstSearch();
			}
	...
	/**
	 * Find extension candidates legs of current subgraphs, then add these
	 * legs into this.candidates
	 * @return
	 */
	/*private void addExtensionCandidate(){
		...
		if(isSubgraphATree()){// tree
			nodes = addNodeExtensionCandidate();
			edge = addEdgeExtensionCandidate();
			//if(edge!=0)
				//System.out.println("In EXTENSION : " + "nodes: " + nodes + " EDGES: "+edge);
		}
		else 
	...
	private boolean isSubgraphATree(){
		if(subgraphNodeNum-1 == subgraphEdgeNum)
			...
			System.out.println("YDY: Error in isSubgraphAtree, inconsistant nodeNum: "+ subgraphNodeNum
					+ "edgeNum: " + subgraphEdgeNum);
		return false;
	}
	/**
	 * Given current subgraph, find a node that is not in current subgraph
	 * but has maximum index of all leaf nodes after adding this node into subgraph
	 * @return
	 */
	/*private int addNodeExtensionCandidate(){
		HashSet<Integer> connectingEdges = new HashSet<Integer>(10);
		// Find maximumIndex and secondMaxIndex of all leaf nodes of current subgraph
		int maximumIndex = -1;
		int secondMaxIndex = -1;
		for(int nodeI = vertices.length-1; nodeI>=0; nodeI--){
			// only interested in leaf node: degree = 1 || 0
		...
		// B. if connected with maximumIndex then it has to be larger than the secondMaxIndex, 
		// B(2). else has to be larger than the maximumIndex
		// C. for single vertex V graph, it candidates can be vertex adjacent to V, and must have a index larger
		// than V
		for(int nodeI = vertices.length-1; nodeI>=0; nodeI--){
			...
			if(subgraphNodesDegree[nodeI]>=1){
				for(int j = 0; j< vertices[nodeI].length;j++){
					...
					// rule A: only interested in adjacent nodes that are not in subgraph yeet
					if(subgraphNodesDegree[nodeJ]==-1)
						// rule B(2) || rule B
						if(nodeJ > maximumIndex||(nodeI == maximumIndex && nodeJ> secondMaxIndex))
							if(connectingEdges.add(connectivity[nodeI][nodeJ]))
								candidates[candidateValidIndex].offer(new Leg(nodeI, nodeJ,Leg.NODE_EXTENSION));
				}
			}
			// rule C
	...
	private int addNodeExtensionCandidate(){
		...
		for(int nodeI = vertices.length-1; nodeI>=0; nodeI--){
			...
			if(subgraphNodesDegree[nodeI]>=1){
				for(int j = 0; j< vertices[nodeI].length;j++){
					int nodeJ = vertices[nodeI][j];
					// rule A: only interested in adjacent nodes that are not in subgraph yet
					if(subgraphNodesDegree[nodeJ]==-1)
						// rule B(2) || rule B
						if(connectingEdges.add(connectivity[nodeI][nodeJ]))
							candidates[candidateValidIndex].offer(new Leg(nodeI, nodeJ,Leg.NODE_EXTENSION));
	...
	/**
	 * Given current subgraph, find a edge that is 
	 * A. not in current subgraph but in query graph q
	 * B. is the largest if added among all nodes in the same cycle
	 * @return
	 */
				...
				if(subgraphNodesDegree[nodeJ] == -1||nodeI > nodeJ)
					...
					if(!nodeJConnected2NodeIinSubgraph){
						...
						if(connectivity[nodeI][nodeJ]>maxIndex[nodeJ]){
							candidates[candidateValidIndex].offer(new Leg(nodeI, nodeJ, Leg.EDGE_EXTENSION));
							count++;
						}
					}		
				}
	...
	/**
	 * Find the maximum edge index along paths starting from nodeA
	 * Doing breath first search
	 * Avoiding information passing cycle
	 * @param nodeA
	 * @return
	 */
	private int[] maxSubgraphEdgeAlongPath(int nodeA){
		...
		while(!queue.isEmpty()){
			...
			for(int j = 0; j< subgraphNodesDegree[nodeI]; j++){
				...
				if(parents[nodeI]== nodeJ)// nodeI is actually connected with nodeA through nodeJ
					continue;
	...
	/**
	 * In the depth first search of subgraphs tree, we need to revisited above level
	 * therefore, have to restore any changed have been made on low level (depth)
	 */
	private void restoreSubgraph(){
		//if(this.subgraphNodeNum == this.subgraphEdgeNum-1)
			//System.out.println("Begining");
		// Restore the leg extension we have made: changing candidateVlidIndex from n to n+1
		...
		if(addedLeg.isNodeOnlyExtension()){
			...
			removeSubgraphEdge(addedLeg.getAlreadyInNode(), addedLeg.getNewNode());

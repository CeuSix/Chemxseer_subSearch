...
public class Leg {
	...
	/**
	 * When extension type is node extension
	 * nodeA is nodes already in subgraph, nodeB is newly added node
	 * When extension type is edge extension
	 * nodeA and nodeB are both nodes that already in subgraph
	 * @param nodeA
	 * @param nodeB
	 * @param extensionType
	 */
	public Leg(int nodeA, int nodeB,  boolean extensionType){
		if(nodeA == -1 || nodeB == -1)
			System.out.println("YDY: Error in Leg construction: not a valid extension");
		this.nodeA = nodeA;
		this.nodeB = nodeB;

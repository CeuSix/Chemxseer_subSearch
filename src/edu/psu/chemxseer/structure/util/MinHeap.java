...
public class MinHeap <T extends Comparable<T>> {
	...
	public void insert(T entry){
		...
		while(currentPosition!=1 && ((T)heap[currentPosition]).compareTo((T)heap[parent(currentPosition)]) == -1){
	...
	private void pushdown(int position){
		...
		while(!isleaf(position)){
			...
			if(smallestchild <  size && ((T)heap[smallestchild]).compareTo((T)heap[smallestchild+1]) ==1)
	...
	private void pushup(int position){
		while(position!=1){
			...
			if(((T)heap[position]).compareTo((T)heap[parent])==-1)

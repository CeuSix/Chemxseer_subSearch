...
public class VariousDensity {
	...
	public void preprocess() throws IOException{
		...
		for(int i = 1; i< 6; i++){
			...
			if(!folder.exists())
				folder.mkdirs();
			PreProcessTools2.covertToSdtGraph(baseName + "50_0." + i + ".data" , folderName + "DBFile");
		}
	}
	
	public static void main(String[] args) throws CorruptIndexException, LockObtainFailedException, IOException, ParseException, MathException{
...
//		}
		...
		for(int i = 1; i< 6; i++){
			...
			BasicExpBuilder builder = new BasicExpBuilder(dbName, MyFactory.getDFSCoder(), baseNameI);
		...
		for(int i = 1; i< 6; i++){
			...
			String dbName = baseName + i + "/DBFile";
			String baseNameI = baseName + i + "/";
			GraphDatabase_OnDisk gDB = new GraphDatabase_OnDisk(dbName, MyFactory.getDFSCoder());

...
/**
 * This is an extension of FastSU
 * It support complete embedding mining & embedding growing 
 * @author dayuyuan
 *
 */
public class FastSUExt {
	private int[][] maps; // The mappings records between the small graph and the big graph
	private int mapsNum;
	...
	/**
	 * Given two graphs small and big
	 * Find all complete embeddings between them
	 * @param small
	 * @param big
	 * @return
	 */
	 ...
	 * Given two graphs small and big
	 * Find all complete embeddings between them
	 * This complete embeddings can be used as 
	 * start point of further embedding extension [middle, big]
	 * @param small
	 * @param big
	 * @return
	 */
	...
	private void findCompleteEmbedding(FastSUState currentState, int SeedcandidateS, int SeedcandidateB){
		//System.out.println(currentState.getDepth());
		currentState.expand(SeedcandidateS, SeedcandidateB);
		
		if(currentState.isGoal()){
			//successfully find a mapping
			this.saveCurrentMatch(currentState.getMap(), currentState.getNodeCountS());
	...
	/**
	 * Based on the start point obtained from stateCompleteEmbeddingExt
	 * A complete embedding between a middle graph (an extension of the small graph)
	 * and the big graph can be found by growEmbeddingExt(extension)
	 * @param oriState
	 * @param extension
	 * @return
	 */
	public FastSUStateExt growCompleteEmbeddingExt(int[][] extension){
		...
		for(int i = 0; i< this.maps.length; i++){
			if(mapsStatus[i] == -1 ||mapsStatus[i]!=currentNodeCountS-1)
				continue; // this map is not valid to index state.getNodeCountS() -1
			else if(mapsEffectiveRound[i] >= currentNodeCountS)
				continue; // this map is not yet valid untill state.getNodeCountS
			// Grow the ith mapping
			else ext = growEmbeddingExt(extension, i);
			if(ext == null)
		...
		else return null; // return null if can not extend
	}
	...
	private boolean completeEmbeddingExt(int mapIndex){
		...
		for(int i = 0; i< SeedcandidateB.length; i++){
			if(findCompleteEmbedding(SeedcandidateS, SeedcandidateB[i], mapIndex, oriMapStatus))
				success = true;
		}
		return success;
	}
	private boolean findCompleteEmbedding(int SeedcandidateS, int SeedcandidateB, 
			int mapIndex, int oriMapStatus){
		//System.out.println(currentState.getDepth());
		...
		if(state.isGoal()){
			...
			if(oriMapStatus == this.mapsStatus[mapIndex])
				this.extendCurrentMatch(state.coreS, mapIndex,oriMapStatus, state.getNodeCountS());
			else // second time find a mapping: duplicate
				this.duplicateMatch(state.coreS, oriMapStatus, state.getNodeCountS());
			state.backTrack();
			success = true;
		}
			...
			for(int i = 0; i< candidateB.length; i++){
				findCompleteEmbedding(candidateS, candidateB[i], mapIndex, oriMapStatus);
			}
			state.backTrack();
		}
		return success;
	...
	/**
	 * Set all mapping that is larger than nodeCount to the size nodeCount
	 * Set all mapping that is not valid untill nodeCount to be invalid
	 * @param nodeCount
	 */
	...
	/**
	 * The first time saves a match into maps array
	 * the maps array grows when its available capacity = 0
	 * @param match
	 */
	private void saveCurrentMatch(int[] match, int nodeIndexUpper){
		saveCurrentMatch(match, 0, nodeIndexUpper);
	}
	private int saveCurrentMatch(int[] match, int nodeIndexLower, int nodeIndexUpper){
		...
		if(availableSlot != -1 && availableSlot < this.mapsStatus.length){
			saveCurrentMatch(match, availableSlot, nodeIndexLower, nodeIndexUpper);
		...
		// No available slots for this new match
			...
			saveCurrentMatch(match, this.mapsNum, nodeIndexLower, nodeIndexUpper);
			return this.mapsNum;
		}
		
	}
	private void saveCurrentMatch(int[] match, int matchIndex, int nodeIndexLower, int nodeIndexUpper){
		if(maps[matchIndex] == null)
			maps[matchIndex] = new int[maps[0].length];
		for(int i = 0; i< nodeIndexUpper; i++){
			//System.out.println("MapsLengh " + maps[mapsNum].length + "mathLength " + match.length);
	 ...
	 * @param matchIndex
	 */
	private void extendCurrentMatch(int[] match, int matchIndex, int nodeIndexLower, int nodeIndexUpper){
	...
	private void duplicateMatch(int[] match, int nodeIndexLower, int nodeIndexUpper){
		int matchIndex = saveCurrentMatch(match, 0, nodeIndexUpper);
		this.mapsEffectiveRound[matchIndex] = nodeIndexLower;
	}
	/**
	 * only the partial mapping from 0 to toNodeIndex (not included is effective)
	 * @param matchIndex
	 * @param toNodeIndex
	 */
	...
	public BitSet getValidStatus() {
		...
		for(int i = 0; i< this.mapsStatus.length; i++)
			if(mapsStatus[i]!=-1 && mapsStatus[i] >=this.state.nodeCountS-1)
				validStatus.set(i, true);
			else validStatus.set(i, false);
		return validStatus;
	}

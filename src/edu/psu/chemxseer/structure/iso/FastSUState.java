...
public class FastSUState {
	...
	// verticesS and verticesB are linked-list representation of graphS and graphB
	protected int[][] verticesS, verticesB;
	// connectivityS and connectivityB are matrix representation of graphS and graphB
	...
	// coreS[i] = j if node i in small graph maps with node j in big graph, coreB[j]=i
	...
	protected int currentDepth; // also the number of pairs in the current mapping
	protected int[] newlyAddedS; // the pair of nodes that were added to the current state
	...
	/**
	 * This function find all candidate pairs in which both
	 * vertices connected with current match
	 * @return
	 */
	protected boolean findMatchableCandidate(){
		// find the vertex of graphS that is unmatched
		// but connected to vertices of current state with minimum depth notation
	...
	/**
	 * @author dayuyuan
	 * feasible test: 3 rules
	 * Two vertices (adjS, adjB) which are adjacent to candidateS and candidateB
	 * 1. If adjS & adjB are in current partial mapping, test branch (adjS, candidateS), (adjB, candidateB)
	 * 2. Num(verticesConnectedS) <= Num(verticesCOnnectedB)
	 * 3. Num(s) - Num(verticesConnectedS) <= Num(b) - Num(verticesCOnnectedB)  
	 * the third rule is only for edge-reduced subgraph-isomorphism test
	 * @return
	 */
	...
	/**
	 * @author dayuyuan
	 * feasible test: 3 rules when candidate is generated from unconnected set
	 * Two vertices (adjS, adjB) which are adjacent to candidateS and candidateB
	 * 1. If adjS & adjB are in current partial mapping, test branch (adjS, candidateS), (adjB, candidateB)
	 * 2. Num(verticesConnectedS) <= Num(verticesCOnnectedB)
	 * 3. Num(s) - Num(verticesConnectedS) <= Num(b) - Num(verticesCOnnectedB)  
	 ...
	 * @return
	 */
	private boolean feasibleTest2(int candidateB){
		// rule 1: No connecting vertices in partial mapping, skip
		// rule 2: Num(verticesConnectedToCurrentState) = 0 , only need to find out
		// the number of vertices connected to candidate, guarantee that non of them
		// in connected set
		int T_S = verticesS[candidateS[currentDepth]].length;
		int T_B = verticesB[candidateB].length; 
		if(T_S > T_B)
			return false;		
		else return true;
	}
	/**
	 * @author dayuyuan
	 * State of VF algorithm grows in a depth first order
	 * When some state is dead, we need to backTrack
	 * Recover the modification of common data member
	 */
	...
	public int getCandidateS() {
		if(currentDepth == this.nodeCountS)
			return -1;
		else return candidateS[currentDepth];
	}
	public int[] getMap(){
		return this.coreS;
	}

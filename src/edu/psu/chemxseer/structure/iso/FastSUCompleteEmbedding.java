...
public class FastSUCompleteEmbedding {
	protected int[][] maps; // The mappings records between the small graph and the big graph
	protected FastSUStateExpandable state;
	private int mapNum;
	private boolean isomorphic;
	// mapChanged = true if some of the original mappings vanished during the mapping extension
	...
	public FastSUCompleteEmbedding(Graph small, Graph big){
		if(small.getEdgeCount() > big.getEdgeCount() || small.getNodeCount() > big.getNodeCount()){
	...
	public FastSUCompleteEmbedding(String smallDFSString, Graph big){
		int[][] GVCodes = MyFactory.getDFSCoder().parseTextToArray(smallDFSString);
	 ...
	 * @param big
	 */
	public FastSUCompleteEmbedding(int[][] smallConnectivity, int[][] smallVertices, Graph big){
		int edgeCountS = 0;
		...
		if(edgeCountS > big.getEdgeCount() || nodeCountS > big.getNodeCount())
			...
			this.mapNum = 0;
			this.isomorphic = false;
			state = new FastSUStateExpandable(smallConnectivity, smallVertices, big);
			completeEmbedding(state);
			this.mapChanged = true;
		}
	}
	/**
	 * If oriEmbedding is expandable with extension, then we build a new FastSUStateExpandable state
	 * and grows the partial mapping to a complete mapping
	 * otherwise, we just assign the internal state to null;
	 * @param oriEmbedding
	 * @param extension
	 */
	public FastSUCompleteEmbedding(FastSUCompleteEmbedding oriEmbedding, int[][] extension){	
		this.isomorphic = false;
		...
		if(state!=null){
			...
			for(int i = 0; i< oriEmbedding.getMapNum(); i++){
				int validMapping = state.replaceMapping(oriMap[i], oriEmbedding.state.nodeCountS-1);
				if(validMapping == 1){
				...
				else if(validMapping == 2){ // subgraph isomorphism test: can not grow
	 ...
	 * @param middleGraph
	 */
	public FastSUCompleteEmbedding(FastSUCompleteEmbedding oriEmbedding, Graph middleGraph){	
		this.isomorphic = false;
		state = oriEmbedding.state.expandToNewState(middleGraph);
		this.mapChanged = false;
		if(state!=null){
			int[][] oriMap = oriEmbedding.maps;
			for(int i = 0; i< oriEmbedding.getMapNum(); i++){
	...
	private boolean completeEmbedding(FastSUState fastSU){
		...
		for(int i = 0; i< SeedcandidateB.length; i++){
			boolean status = findCompleteEmbedding(fastSU, SeedcandidateS, SeedcandidateB[i]);
			if(status)
				success = true;
		}
		return success;
	...
	private boolean findCompleteEmbedding(FastSUState currentState, int SeedcandidateS, int SeedcandidateB){
		//System.out.println(currentState.getDepth());
		currentState.expand(SeedcandidateS, SeedcandidateB);
		boolean success = false; // denote whether we find at least one embedding
		if(currentState.isGoal()){
			...
			for(int i = 0; i< candidateB.length; i++){
				boolean status = findCompleteEmbedding(currentState, candidateS, candidateB[i]);
				if(status)
					success = status;
			}
			currentState.backTrack();
	...
	public FastSUStateExpandable getState() {
		if(this.maps == null || maps.length == 0){
			...
			int validMapping = result.replaceMapping(this.maps[0], state.nodeCountS-1);
			return result;
		}
	}

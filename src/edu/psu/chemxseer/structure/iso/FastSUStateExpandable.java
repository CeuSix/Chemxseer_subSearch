...
/**
 * A implementation of FastSUState, where the smaller graph can be expanded to a middle graph
 * or shrink to a smaller graph
 * @author dayuyuan
 *
 */
public class FastSUStateExpandable extends FastSUState{
	 ...
	 * @param big
	 */
	public FastSUStateExpandable(int[][] smallConnectivity, int[][] smallVertices, Graph big){
		super();//dummy
		// Initial internal representation of graph small and graph big
	 ...
	 * The internal big graph representation 
	 * can be shared among all FastSUStateExpandable
	 * @param small
	 * @param big
	 */
	...
	protected void graphInitial(int[][] smallConnectivity, int[][] smallVertices, Graph big){
	...
	/**
	 * All those internal data member are large enough to be shared among all state
	 */
	...
	protected boolean graphExtension(FastSUStateExpandable oriSTATE, int[][] extension){
		// share the big graph 
	...
	protected boolean graphExtension(FastSUStateExpandable oriSTATE, Graph middleGraph){
		// share the big graph 
		this.verticesB = oriSTATE.verticesB;
		this.connectivityB = oriSTATE.connectivityB;
		// Do extending the smaller graph
		...
		for(int i = 0; i< verticesS.length; i++){
			...
			for(int j = 0; j< verticesS[i].length; j++)
			{
				if(coreS[verticesS[i][j]] !=NULL_NODE)
					continue;
				else connectS[verticesS[i][j]] = this.currentDepth-1;
			}
		}
		for(int i = 0; i< verticesB.length; i++){
			...
			for(int j = 0; j< verticesB[i].length; j++)
			{
				if(coreB[verticesB[i][j]] !=NULL_NODE)
					continue;
				else connectB[verticesB[i][j]] = this.currentDepth-1;
			}
		}
	}
	
	/**
	 * Return -1: the replaced mapping is not valid
	 * Return  0: the replaced mapping is perfect mapping. [small = big]
	 * Return  1: the replaced mapping is partial mapping.
	 * Return  2: the replaced mapping is full mapping. 
	 * 	[All small graphs nodes are mapped to nodes in the big graph]
	 * @param newCoreS
	 * @param validLength
	 * @return
	 */
	...
	/**
	 * Given the current core mapping between small graph and large graph
	 * test whether it is correct
	 * Return -1 if not valid
	 * Return 0 if is a full mapping
	 * Return 1 if a partial valid mapping
	 * @return
	 */
	private int validateNewMapping(){
		...
		for(int i = 0; i< verticesS.length; i++)
			if(this.coreS[i]!=NULL_NODE)
				mappedNum++;
		if(this.edgeNumS == this.edgeNumB && this.nodeCountS ==this.nodeCountB && mappedNum == this.nodeCountS )
			return 0; // perfect match, an isomorphism test is found
		else if(mappedNum == this.nodeCountS)
			return 2; // it is alreay a complete match, but only subgrahp isomorpihsm
		return 1;
	}

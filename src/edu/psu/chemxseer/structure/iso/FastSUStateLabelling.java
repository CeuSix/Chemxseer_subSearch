...
/**
 *FastSUStateLabelling is one extension of FastSUState
 *It support relabel the big graph as an extension of the small graph
 *if small subgraph is subgraph isomorphic to the big graph
 *After extension relabeling, it can further permuted inner representation 
 *of the big graph. This permuted big graph can be further utilized
 *when convert a larger graph as an extension of the big graph
 * @author dayuyuan
 *
 */
public class FastSUStateLabelling extends FastSUState{
	...
	/**
	 *Given a relabelled isomorphism between graph small and graph big
	 *now try to relabel graph large as an isomorphic extension to graph big 
	 */
	public FastSUStateLabelling(FastSUStateLabelling oneIsomorphism, Graph large){
		nodeCountS = oneIsomorphism.nodeCountB;
		nodeCountB = large.getNodeCount();
		// Initial internal representation of graph small and graph big
		graphInitial(oneIsomorphism.verticesB.clone(), oneIsomorphism.connectivityB.clone(), large);
		datamemberInitial();
		this.findInitialPair();
	...
	/**
	 * 
	 * @param oneIsomorphism: the isomorphism between supper small graph & small graph
	 * @param labels: label for the large graph
	 */
	public FastSUStateLabelling(FastSUStateLabelling oneIsomorphism, int[][] big){
		...
		nodeCountB++;
		// Initial internal representation of graph small and graph big
		graphInitial(oneIsomorphism.verticesB.clone(), oneIsomorphism.connectivityB.clone(), big);
		datamemberInitial();
		this.findInitialPair();
		edgeNumS = oneIsomorphism.edgeNumB;
		edgeNumB = big.length;
	...
	protected void graphInitial(int[][] vertices, int[][] connectivity, Graph big){
	...
	protected void graphInitial(int[][] vertices, int[][] connectivity, int[][] largeLabels){
		this.verticesS = vertices;
		this.connectivityS = connectivity;
	...
	/**
	 * Relabel Graph B as an extension of Graph S
	 * Save those extension into extension
	 * return 
	 * @param extension
	 * @return
	 */
	protected void relabelGraphB(){
		...
		for(int nodeB = 0; nodeB < nodeCountB; nodeB++)
		{
			...
			for(int j = 0; j< verticesB[nodeB].length; j++){
				...
				if(layer[adjNodeB]==NULL_NODE)// Find an adjacent node that are not in small graph
				{
					layer[adjNodeB] = 1;
					order[adjNodeB] = verticesIndex++;
					queue.offer(adjNodeB);
				}
				if(layer[nodeB] < layer[adjNodeB]|| layer[nodeB] == layer[nodeB] && order[nodeB] < order[adjNodeB]
				       &&connectivityS[coreB[nodeB]][coreB[adjNodeB]]==NULL_NODE && connectivityB[nodeB][adjNodeB]!=NULL_NODE)
				{
					// 1, layer[nodeB] < layer [adjNodeB] 2, layer equals, order[nodeB] < order[adjNodeB]
		...
		while(!queue.isEmpty()){
			...
			while(node !=-1){
				for(int j = 0; j< verticesB[node].length; j++){
					...
					//layer[nodeB] < layer[adjNodeB]|| layer[nodeB] == layer[nodeB] && order[nodeB] < order[adjNodeB]
					// equals order[node] < order[adjNodeB]
					// since both node and adjNodeB are not in small graph, thus connectivityS[ndoeB][adjNodeB] ==NULL_NODE
	...
	protected void reOrderGraphB(){
		...
		for(int nodeB = 0; nodeB < nodeCountB; nodeB++){
			...
			for(int i = 0; i < verticesB[nodeB].length; i++){
				...
				newConnectivityB[reorderB][order[adjNodeB]] = 
					connectivityB[nodeB][adjNodeB];

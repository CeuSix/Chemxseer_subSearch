...
public class FastSU {
	...
	public boolean isIsomorphic(Graph small, Graph big){
		if(small.getNodeCount() > big.getNodeCount() || small.getEdgeCount() > big.getEdgeCount())
			return false;
		FastSUState fastSUState = new FastSUState(small, big);
		return isIsomorphic(fastSUState);
	}
	
	public boolean isIsomorphic(FastSUCompleteEmbedding oriEmbedding, int[][] extension){	
		FastSUStateExpandable state = oriEmbedding.state.expandToNewState(extension); // create an new state
		// The new state maybe : node extension of the oriState, or edge extension of the oriState, or both
		if(state!=null){
			...
			for(int i = 0; i< oriEmbedding.getMapNum(); i++){
				...
				if(validMapping == 1){
					...
					// the mapping is not correct: this is because the (1) the extension is an edge extension (2) the mapping is not valie after
	...
	protected boolean isIsomorphic(FastSUState fastSUState){
		...
		for(int i = 0; i< SeedcandidateB.length; i++){
			boolean success = startMatching(fastSUState, SeedcandidateS, SeedcandidateB[i]);
			if(success)
				return success;
		}
		return false;
	...
	private boolean startMatching(FastSUState currentState, int SeedcandidateS, int SeedcandidateB){
		...
		if(currentState.isGoal()){
			...
			for(int i = 0; i< candidateB.length; i++){
				boolean match = startMatching(currentState, candidateS, candidateB[i]);
				if(match)
					return true;
				else continue;
			}
	...
	/**
	 * Given the small graph labels (represented as in LindexTerm extension), and the
	 * big graph 
	 * @param labels
	 * @param big
	 * @return
	...
	public FastSUStateLabelling graphExtensionLabeling(int[][] small, int[][] big){
		FastSUStateLabelling fastSU = new FastSUStateLabelling(small, big); 
		boolean isomorphic = this.isIsomorphic(fastSU);
		// small is not a subgraph of big, return null
		if(isomorphic == false)
	...
	public FastSUStateLabelling graphExtensionLabeling(FastSUStateLabelling oriFastSU, Graph big){
	...
	public FastSUStateLabelling graphExtensionLabeling(FastSUStateLabelling oriFastSU, int[][] bigLabel){
		FastSUStateLabelling fastSU = new FastSUStateLabelling(oriFastSU, bigLabel); 
		boolean isomorphic = this.isIsomorphic(fastSU);
		// small is not a subgraph of big, return null
		if(isomorphic == false)
			return null;

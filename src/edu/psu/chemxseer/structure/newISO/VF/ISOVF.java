	...
	protected boolean match(VFState currentState){
		...
		if(currentState.isGoal())
			...
			for(int i = 0; i< candidateB.length; i++){
				// Compute the state s' obtained by adding (candidateS, B) to M(s)
				VFState newState = new VFState(currentState, candidateS, candidateB[i]);
				if(match(newState))
					return true;
				else{
					// Restore the common shared data member to state: currentState
					newState.restore();
					newState = null;
					continue;
				}

...
public class VFState {
	...
	// The depth in the SSR tree of the state in which the node entered the corresponding set
	...
	public VFState(VFState prestate, int candidateS, int candidateB){
		...
		// Update common shared data member after adding the new pair(candidateS, candidateB)
	...
	public boolean restore(){
		...
		for(int i = 0; i< graphS.getDegree(newlyAddedS); i++){
			int adjNodeS = graphS.getAdjacentNode(newlyAddedS, i);
			if(coreS[adjNodeS] == NULL_NODE && connectS[adjNodeS] == currentDepth){
		...
		for(int j = 0; j< graphB.getDegree(newlyAddedB); j++){
			...
			if(coreB[adjNodeB] == NULL_NODE && connectB[adjNodeB] == currentDepth){
	...
	/**
	 * Generate the search order of subgrpah isomorphism test
	 * The first step of computation of P(s), finding candidateS
	 * @param order
	 * @return
	 */
	protected boolean findSearchOrder(){
		...
		while(mHeap.isEmpty()!=true){
			...
			orderSize++;
			// add all nodes adjacent to the newly added candidateS into the minHeap
			for(int i = 0; i< graphS.getDegree(nodeIndex); i++)
			{
				...
				mHeap.offer(new NodeInfoVF(adjNode, graphS.getNodeLabel(adjNode) ));
			}
		}	
		return true;
	}
	...
	/**
	 * The second step of computation of P(s)
	 * With a fixed candidateS = searchorder[currentDepth]
	 * Find the set of candidateB
	 * @return
	 */
	public int[] getCandidateB(){
		...
		// the seed state: candidateB can be any node with the same label of candidateS
		if(currentDepth ==0){
		...
		// candidateB must connected with the node, adjNodeB, which is the matching node of 
		// adjNodeS in graphB. 
		else{
	...
	/* FeasibleTest:
	   Two vertices (node1, node2) 
	   1. If adjNode1 & adjNode2 are in current partial match, 
     	test edge (node1, adjNode1), (node2, adjNode2)
	...
	private boolean isFeasible(int node1, int node2){
		...
		for(int i = 0; i< graphS.getDegree(node1); i++){
			...
			if(coreS[adjNode1]==NULL_NODE)
				...
				if(graphS.getEdgeLabel(node1, adjNode1)!=graphB.getEdgeLabel(node2, adjNode2))
					return false;
			}
		}
		// rule 2

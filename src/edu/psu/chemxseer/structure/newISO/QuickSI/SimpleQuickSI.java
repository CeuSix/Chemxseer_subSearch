...
/**
 * An implementation of the quickSI algorithm, a simply version
 * Implementation details are not covered in the paper
 * @author dayuyuan
 *
 */
public class SimpleQuickSI {
	...
	protected boolean[] F;
	//Array H is used to record the mapping between vertices in graph S and graph B 
	...
	public boolean QuickSITest(){
		if(this.currentDepth == graphS.getNodeCount())
			...
			// find all nodes in graphB that is matchable with the first node of graphA
	...
	public boolean match(int sequenceIndex, int nodeIndexB){
		...
		// label
		if(sequence.getEntry(sequenceIndex).getNodeLabel()!=graphB.getNodeLabel(nodeIndexB))
			return false;
		else{
			ConstrainEntry[] constrains = sequence.getEntry(sequenceIndex).getConstrains();
			if(constrains == null)
				return true;
			else{
				for(int i = 0; i< constrains.length; i++){
					...
					if(constrains[i].isDegreeContrain() && 
							constrains[i].getContrainValue() > graphB.getDegree(nodeIndexB))
						return false;
					// backward edge constrain
					else if(constrains[i].isEdgeContrain()){
						int aheadNodeS = this.sequence.getEntry(constrains[i].getContrainValue()).getNodeID();
						int aheadNodeB = this.H[aheadNodeS];
						if(aheadNodeB == -1 || graphS.getEdgeLabel(aheadNodeS,nodeIndexS)
								!= graphB.getEdgeLabel(aheadNodeB, nodeIndexB));
							return false;
					}
				}
				return true;

...
import edu.psu.chemxseer.structure.util.MinHeap;

/**
 * An implementation of the QISequence
 * Given a query graph q, a QI-Sequence is a sequence that represents a
 * rooted spanning tree for q. 
 * @author dayuyuan
 *
 */
public class QISequence implements QISequenceInterface{
	// A QI Sequence is an Array of QISequenceEntry, which is derived from a query graph
	...
	/**
	 * This is simplified version of building a QISequence
	 * Construct the QISequence by A depth first search of the underlying graph
	 */
	private void buildSequence(){
		...
		if(graph.getDegree(0) >2)
			firstEntry.addConstrain(ConstrainEntry.buildDegreeConstrain(graph.getDegree(0)));
		...
		while(!dfs.isEmpty()){
			...
			if(map[currentNode]==-1){
				// find a forward edge
				//System.out.println("index " + index + " currentNode " + currentNode);
				currentEntry = new ForwardEntry(currentNode, parents[currentNode], graph.getNodeLabel(currentNode));
			...
			if(degree> 2)
				currentEntry.addConstrain(ConstrainEntry.buildDegreeConstrain(degree));
			...
			for(int i =0; i<degree; i++){
				...
				if(map[childNode]!=-1 && map[childNode] < index-1){
					...
					currentEntry.addConstrain(ConstrainEntry.buildEdgeConstrain(map[childNode]));
				}
				else{
					dfs.push(childNode);
	...
	/**
	 * Build a QISequence that will produce a effective search order while running the
	 * subgrpah isomorphism test
	 * 1st step: the average inner support of an edge e (or a vertex v) is the average number
	 * of its possible mappings in the graphs which contain this edge (vertex)
	 * 2nd step: finding the minimum spanning tree, extend the Prim's algorithm
	 */
	private void buildSequence2(ISOEdgeGraph query, GraphDBFrequency graphDB){
		...
		PriorityQueue<QIEdge> mHeap= new PriorityQueue<QIEdge>(query.getNodeCount());
		...
		for(int i = 0; i< query.getDegree(firstNodes[0]); i++){
			...
			if(edgeVisited[adjEdge] == true)
				...
				float edgeSupport = graphDB.getEdgeSupport(query.getNodeLabel(firstNodes[0]),
				...
				for(int j = 0; j< query.getDegree(adjNode); j++)
					if(nodeVisited[query.getAdjacentNode(adjNode, j)] != -1)
						indgSize++;
				QIEdge oneISOEdge = new QIEdge(adjEdge,edgeSupport, indgSize, query.getDegree(adjNode));
		...
		while(!mHeap.isEmpty()){
			...
			this.sequence[index] = new ForwardEntry(toBeaddedNode, nodeVisited[alreadyInNode], 
			...
			if(query.getDegree(toBeaddedNode) > 2)
				this.sequence[index].addConstrain(ConstrainEntry.buildDegreeConstrain
			...
			for(int i = 0; i< query.getDegree(toBeaddedNode); i++){
				...
				if(edgeVisited[adjNode])
					...
					this.sequence[index].addConstrain(ConstrainEntry.buildEdgeConstrain(
					nodeVisited[adjNode]));
					...
					for(int w = 0; w < query.getDegree(adjNode); w++)
						if(nodeVisited[query.getAdjacentNode(adjNode, w)] != -1)
							indgSize++;
					mHeap.offer(new QIEdge(adjEdge,edgeSupport, indgSize, query.getDegree(adjNode)));
				}
			}
			index++;
		}	
	...
	private int selectFirstEdge(ISOEdgeGraph query, GraphDBFrequency graphDB){
		...
		for(int i  = 0; i< query.getEdgeCount(); i++){
			...
			float support = graphDB.getEdgeSupport(labelNode1, labelNode2, query.getEdgelabel(i));
			if(support < minWeight){
				minEdge = i;
				minWeight = support;
				minDegree = query.getDegree(nodes[0]) + query.getDegree(nodes[1]);
			}
			else if(support == minWeight){
				int newDegree = query.getDegree(nodes[0]) + query.getDegree(nodes[1]);
		...
		this.sequence[0] = new ForwardEntry(startPoint, -1, query.getNodeLabel(startPoint));
		if(query.getDegree(startPoint) > 2)
			this.sequence[0].addConstrain(ConstrainEntry.buildDegreeConstrain(query.getDegree(startPoint)));
		// construct the second QIEntry
		this.sequence[1] = new ForwardEntry(endPoint, 0, query.getNodeLabel(endPoint));
		if(query.getDegree(endPoint) > 2)
			this.sequence[1].addConstrain(ConstrainEntry.buildDegreeConstrain(query.getDegree(endPoint)));
		return minEdge;
	}
}

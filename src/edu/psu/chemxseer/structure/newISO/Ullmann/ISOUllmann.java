...
/**
 * An implementation of the Ullmann algorithm for subgraph isomorphism test
 * This is just an iterative implementation instead of recursion based
 * @author dayuyuan
 *
 */
public class ISOUllmann {
	...
	// Ullmann algorithm arrange the search order correspond respectively to the points 
	// of graphS in order of decreasing degree.
	int[] indexS; //indexS[index] = node
	int[] nodeS; // nodesS[node] = index
	//Array F is used to record which vertices in graphB has already been mapped
	//if F[i] = true, then graphB(i) is used, i is the index of the node
	protected boolean[] F;
	//Array H is used to record the mapping between vertices in graphS and graphB 
	//if H[i]= k, then S(i) matched with B(k), i is index number, not node
	protected int[] H;
	//M records the partial mapping between S and B
	protected boolean[][] M; //M[S][B] = true means the node S and B can be mapped.The current partial mapping
	protected boolean[][][] MD; // All partial mapping in different levels.
	// For initialM, the first alreadyMappedIndex number of nodes has been mapped successfully due to 
	// previous knowledge, isomorphism test start from the initialM[alreadymappedIndex]
	...
	/**
	 * A self implementation of bucket sort
	...
	protected void bucketSort(int[] oriData, int[] afterData, int[] rafterData){
		...
		List<List<Integer>> buckets = new ArrayList<List<Integer>>(max-min+1);
	...
	/**
	 * Do subgraph isomorphism test between GraphS and GraphB
	...
	protected boolean isIsomorphic(int startingDepth){
		...
		for(int d = startingDepth; d< nodeCountS; d++){
			...
			// try to find a vertex K in graphB, which is matchable to vertex d in graphS
			// If vertex d is never matched to any vertex(traceBack = false), start search k from 0
			...
			if(traceBack){
				// any previous operation on M is taken back, especially done by refinement process
			...
			if(findAK == true)
			{
				...
				H[d] = k;
				F[k] = true;
				if(d+1 == nodeCountS) return true;//find a successful match
				else if(refine()){
					traceBack = false;// keep on growing
					assign(MD[d], M, nodeCountS, nodeCountB);// save MD[d] for backup before refinement process
					continue; // continue searching for next vertex in graphS (d+1)
				}
			...
			// No K is available: trace back
				...
				if(d==startingDepth) return false;
	...
	/**
	 * Refinement process of early detection of failure partial match
	 * Refinement rule: for all vertices that has been matched, their adjacent vertices should
	 * also be matchable.
	 * @return
	 */
	...
	/**
	 * The crux of ullman algorithm
	 * At any media state, if nodeS and nodeB are newly matched
	 * Their adjacent vertices have to matched and also their adjacent edges
	 * @param nodeS
	 * @param nodeBArray
	 * @return
	 */
	protected boolean canMatch(int sindex,int bnode){
		...
		for(int i = 0; i<degreeS; i++){
			...
			if(this.H[adjIndexS] !=-1)// this vertex has already been mapped
			{
				...
				if(graphS.getNodeLabel(adjNodeS)==graphB.getNodeLabel(adjNodeB))
				...
				for(int j = 0; j<graphB.getDegree(bnode); j++){
					...
					if(M[adjIndexS][adjNodeB]==false)
						continue;
					// adjS maps with adjB, test edge(nodeS, adjS) and edge(nodeB, adjB)
					else if(graphS.getEdgeLabel(snode, adjNodeS)!= graphB.getEdgeLabel(bnode, adjNodeB))
						continue;
					else{
						itherAdjHasAMap = true;
						break;
	 ...
	 * @return
	 */
	protected boolean assign(boolean empty[][], boolean data[][], int Xsize, int Ysize){

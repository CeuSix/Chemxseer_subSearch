...
/**
 * Ullmann algorithm: isomorphism testing one query graph with a list of target graphs
 * Compared to ISOUllmann, ListUllmann saved memory by storing only one copy of data member
 * Those data member are reused for each graphB
 * @author dayuyuan
 *
 */
	...
	protected boolean canMatch(int sindex,int bnode, int ith){
		...
		for(int i = 0; i<degreeS; i++){
			...
			if(this.H[adjIndexS] !=-1)// this vertex has already been mapped
			{
				...
				if(graphS.getNodeLabel(adjNodeS)==graphBs[ith].getNodeLabel(adjNodeB))
				...
				for(int j = 0; j<graphBs[ith].getDegree(bnode); j++){
					...
					else if(graphS.getEdgeLabel(snode, adjNodeS)!= graphBs[ith].getEdgeLabel(bnode, adjNodeB))
						continue;
					else{
						itherAdjHasAMap = true;
						break;
